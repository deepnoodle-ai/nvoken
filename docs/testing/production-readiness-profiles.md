...
| --- | --- | --- |
| `single_daemon` | **Pending** | The runtime mode is implemented, but the packaged install, restart/failure, upgrade/rollback, restore, diagnosis, capacity, retention, and secret-handling evidence below has not been recorded for this profile. |
| `google_cloud` | **Pending** | The paved topology and offline Terraform checks exist, but the required live qualification, upgrade/rollback, restore, operational, retention, and secret-handling evidence below has not been recorded for this profile. |
...
| Retention | Verify authoritative rows remain retained, terminal dispatch/callback diagnostics prune in bounded batches, and Cloud SQL storage signals expose total growth without transcript content. | nvoken: retain-by-default schema and diagnostic pruning. Operator: Cloud SQL storage/backup capacity and future deletion policy. | automated | pending | missing | [PRD 021 A1-A4](../prds/021-prd-initial-retention-posture.md#acceptance) |
