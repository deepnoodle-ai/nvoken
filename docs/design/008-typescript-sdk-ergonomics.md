...
The first recommendation is to define the desired TypeScript contract before
implementing the individual changes in this review. Within that contract, keep
the Agent-first design and make the level of control explicit in method names:
...
## Proposal: design the TypeScript contract first

Do not implement the findings below as a queue of independent renames. First
write and approve a small set of runnable TypeScript workflows for one-shot
text, a tenant conversation, host tools, streaming, admission-only work, and
recovery. Treat those workflows and their public types as the product
contract. Then map them onto the HTTP API and change the wire contract only
where the desired SDK cannot preserve the required behavior safely.
...
const support = client
  .forTenant(customer.id)
  .agent("support", { tools: [lookupOrder] });

const conversation = support.bindSession(ticket.id, {
...
The exact method names remain part of the design exercise. The important
change is that the common path expresses application concepts while the SDK
performs the wire mapping:

| Developer-facing concept | Proposed SDK spelling | Current wire mechanism |
| --- | --- | --- |
| Customer or workspace | `client.forTenant(customer.id)` | `tenant_key` |
| Ordinary Agent | `.agent("support")` | `agent_key` plus a matching `definition_key` default |
| Conversation | `.bindSession(ticket.id)` | `session_key` |
...
Surface-first design does not allow the SDK to fake server guarantees. For
each proposed workflow, record its HTTP calls and failure behavior. If a
...
const chat = support.bindSession(ticket.id, {
  retention: { ttlSeconds: 30 * 24 * 60 * 60 },
});
...
commit. The names, return types, docs, and wire capabilities describe one
mental model.

1. Approve runnable north-star TypeScript workflows and their public types.
   Keep key-suffixed fields out of the ordinary workflow.
2. Map each workflow to HTTP calls, retry behavior, atomicity requirements,
   and recovery behavior. Identify which gaps require an OpenAPI and Runtime
   change rather than SDK adaptation.
3. Settle `start()`, `bindSession()`, the common result type, scoping, and the
   generated transport boundary as one public API decision.
4. Update the HTTP API and Runtime for any behavior the settled surface cannot
   implement safely over the current contract. Regenerate clients before
   changing the handwritten facade.
5. Update TypeScript types so the accepted call shapes are enforced at compile
   time.
6. Preserve handles and idempotency keys on local timeout errors.
7. Add reduced streaming to the Agent and bound Session before changing the
   default stream name.
8. Rewrite the README around the settled vocabulary, then update executable
