...
| 7 | `prd-recovery-and-transcript-reads` | Contract + build | Complete the authoritative recovery surface: Invocation and Session get/list filters, aggregate usage/provenance, pending/active state, cursor-paginated transcript, and a fixed-cut incremental transcript snapshot that orders message changes before terminal Invocation changes. Recovery requires durable IDs only. This read model is the source for SDK reducers and the later stream. | Planned |
