...
/**
 * The one App-wide admission contract shared by machine and browser
 * callers. Both positive values are required and enforced at admission.
 *
 * @export
 * @interface AppDefaultRateLimits
 */
export interface AppDefaultRateLimits {
    /**
     *
     * @type {number}
     * @memberof AppDefaultRateLimits
     */
...
/**
 * Check if a given object implements the AppDefaultRateLimits interface.
 */
export function instanceOfAppDefaultRateLimits(value: object): value is AppDefaultRateLimits {
    if (!('maxConcurrentInvocations' in value) || value['maxConcurrentInvocations'] === undefined) return false;
    if (!('maxAdmissionsPerMinute' in value) || value['maxAdmissionsPerMinute'] === undefined) return false;
    return true;
}

export function AppDefaultRateLimitsFromJSON(json: any): AppDefaultRateLimits {
    return AppDefaultRateLimitsFromJSONTyped(json, false);
}

export function AppDefaultRateLimitsFromJSONTyped(json: any, ignoreDiscriminator: boolean): AppDefaultRateLimits {
    if (json == null) {
        return json;
    }
    return {

        'maxConcurrentInvocations': json['max_concurrent_invocations'],
        'maxAdmissionsPerMinute': json['max_admissions_per_minute'],
    };
}

export function AppDefaultRateLimitsToJSON(json: any): AppDefaultRateLimits {
    return AppDefaultRateLimitsToJSONTyped(json, false);
}

export function AppDefaultRateLimitsToJSONTyped(value?: AppDefaultRateLimits | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }
    ...
    return {
        ...
        'max_concurrent_invocations': value['maxConcurrentInvocations'],
