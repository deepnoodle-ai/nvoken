 ...
 * Text-only canonical history. A string is shorthand for one text block;
 * both forms normalize to the same stored content-block array.
 *
 * @export
 */
export type SeedMessageContent = Array<TextInputBlock> | string;

export function SeedMessageContentFromJSON(json: any): SeedMessageContent {
    return SeedMessageContentFromJSONTyped(json, false);
}

export function SeedMessageContentFromJSONTyped(json: any, ignoreDiscriminator: boolean): SeedMessageContent {
    if (json == null) {
        return json;
    }
    if (Array.isArray(json)) {
        if (json.every(item => typeof item === 'object')) {
            if (json.every(item => instanceOfTextInputBlock(item))) {
                return json.map(value => TextInputBlockFromJSONTyped(value, true));
            }
        }
        return json;
    }
...
export function SeedMessageContentToJSON(json: any): any {
    return SeedMessageContentToJSONTyped(json, false);
}

export function SeedMessageContentToJSONTyped(value?: SeedMessageContent | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }
    if (Array.isArray(value)) {
        if (value.every(item => typeof item === 'object')) {
            if (value.every(item => instanceOfTextInputBlock(item))) {
                return value.map(value => TextInputBlockToJSON(value as TextInputBlock));
            }
        }
        return value;
    }
