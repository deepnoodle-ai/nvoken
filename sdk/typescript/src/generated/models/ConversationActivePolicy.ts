...
/**
 * Selects what admission does when the Conversation already has a
 * nonterminal Turn. `reject` returns `conversation_active` without
 * changing either Turn. `supersede` atomically cancels the predecessor,
 * excludes its messages from later generation, and admits the successor.
 * `interrupt` preserves the predecessor's completed work: it settles the
 * predecessor at a safe seam and admits the successor atomically, or
 * durably requests interruption from its live executor, waits boundedly
 * for settlement, and then retries the same materialized admission. A
 * predecessor that does not settle before that bound returns
 * `conversation_active`; no successor Turn or MemorySpace is created.
 *
 * App credentials may use all three policies. Browser grants scoped to a
 * user's Conversations may use `reject` or `interrupt`. Exact-Conversation
 * browser grants and anonymous grants use `reject`; browser admission
 * never permits `supersede`.
 *
 * @export
 */
export const ConversationActivePolicy = {
    Reject: 'reject',
    Supersede: 'supersede',
    Interrupt: 'interrupt'
} as const;
export type ConversationActivePolicy = typeof ConversationActivePolicy[keyof typeof ConversationActivePolicy];


export function instanceOfConversationActivePolicy(value: any): boolean {
    for (const key in ConversationActivePolicy) {
        if (Object.prototype.hasOwnProperty.call(ConversationActivePolicy, key)) {
            if (ConversationActivePolicy[key as keyof typeof ConversationActivePolicy] === value) {
                return true;
            }
        }
    }
...
export function ConversationActivePolicyFromJSON(json: any): ConversationActivePolicy {
    return ConversationActivePolicyFromJSONTyped(json, false);
}

export function ConversationActivePolicyFromJSONTyped(json: any, ignoreDiscriminator: boolean): ConversationActivePolicy {
    return json as ConversationActivePolicy;
}

export function ConversationActivePolicyToJSON(value?: ConversationActivePolicy | null): any {
    return value as any;
}

export function ConversationActivePolicyToJSONTyped(value: any, ignoreDiscriminator: boolean): ConversationActivePolicy {
    return value as ConversationActivePolicy;
}
