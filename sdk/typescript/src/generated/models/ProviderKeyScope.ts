...
/**
 * `app` is one provider key shared by every tenant of one app;
 * `tenant` is one provider key for a single named
 * tenant_key inside the app.
 *
 * @export
 */
export const ProviderKeyScope = {
    App: 'app',
    Tenant: 'tenant'
} as const;
export type ProviderKeyScope = typeof ProviderKeyScope[keyof typeof ProviderKeyScope];


export function instanceOfProviderKeyScope(value: any): boolean {
    for (const key in ProviderKeyScope) {
        if (Object.prototype.hasOwnProperty.call(ProviderKeyScope, key)) {
            if (ProviderKeyScope[key as keyof typeof ProviderKeyScope] === value) {
                return true;
            }
        }
    }
...
export function ProviderKeyScopeFromJSON(json: any): ProviderKeyScope {
    return ProviderKeyScopeFromJSONTyped(json, false);
}

export function ProviderKeyScopeFromJSONTyped(json: any, ignoreDiscriminator: boolean): ProviderKeyScope {
    return json as ProviderKeyScope;
}

export function ProviderKeyScopeToJSON(value?: ProviderKeyScope | null): any {
    return value as any;
}

export function ProviderKeyScopeToJSONTyped(value: any, ignoreDiscriminator: boolean): ProviderKeyScope {
    return value as ProviderKeyScope;
}
