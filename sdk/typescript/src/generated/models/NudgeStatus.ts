...
/**
 * `pending` is the only state a turn will still pick up. `drained` means
...
export function instanceOfNudgeStatus(value: any): boolean {
    for (const key in NudgeStatus) {
        if (Object.prototype.hasOwnProperty.call(NudgeStatus, key)) {
            if (NudgeStatus[key as keyof typeof NudgeStatus] === value) {
                return true;
            }
        }
    }
...
export function NudgeStatusFromJSON(json: any): NudgeStatus {
    return NudgeStatusFromJSONTyped(json, false);
}

export function NudgeStatusFromJSONTyped(json: any, ignoreDiscriminator: boolean): NudgeStatus {
    return json as NudgeStatus;
}

export function NudgeStatusToJSON(value?: NudgeStatus | null): any {
    return value as any;
}

export function NudgeStatusToJSONTyped(value: any, ignoreDiscriminator: boolean): NudgeStatus {
    return value as NudgeStatus;
}
