...
export class InvocationsApi extends runtime.BaseAPI {
    ...
    /**
     * Creates request options for deleteInvocation without sending the request
     */
    async deleteInvocationRequestOpts(requestParameters: DeleteInvocationRequest): Promise<runtime.RequestOpts> {
        if (requestParameters['invocationId'] == null) {
            throw new runtime.RequiredError(
                'invocationId',
                'Required parameter "invocationId" was null or undefined when calling deleteInvocation().'
            );
        }
    ...
    /**
     * Erases the private content carried by a terminal standalone turn while retaining its minimal usage, lineage, and idempotency facts. A conversation-bound Invocation is erased through its Session instead.
     * Erase a terminal standalone Invocation
     */
    async deleteInvocationRaw(requestParameters: DeleteInvocationRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<void>> {
        const requestOptions = await this.deleteInvocationRequestOpts(requestParameters);
        const response = await this.request(requestOptions, initOverrides);
     ...
     * Erase a terminal standalone Invocation
     */
    async deleteInvocation(requestParameters: DeleteInvocationRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<void> {
        await this.deleteInvocationRaw(requestParameters, initOverrides);
    }
    ...
    async listInvocationsRequestOpts(requestParameters: ListInvocationsRequest): Promise<runtime.RequestOpts> {
        ...
        if (requestParameters['parentInvocationId'] != null) {
            queryParameters['parent_invocation_id'] = requestParameters['parentInvocationId'];
        }
    ...
    /**
     * Carries only this turn\'s durable transcript updates and optional live previews, then closes after its terminal change is delivered. The cursor is scoped to the Invocation and never reveals an internal carrier Session for a standalone turn.
     * Follow one Invocation over Server-Sent Events
     */
    async streamInvocationRaw(requestParameters: StreamInvocationRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<SessionStreamEvent>> {
     ...
     * Follow one Invocation over Server-Sent Events
     */
    async streamInvocation(requestParameters: StreamInvocationRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<SessionStreamEvent> {
