...
export class AppsApi extends runtime.BaseAPI {
    ...
    /**
     * Registers one host application and creates its default tenant, returning the generated `app_id` and independent callback and webhook HMAC keys. The plaintext signing keys are returned only in this response; store them in the receiver\'s secret manager. nvoken stores only authenticated ciphertext and selects a key from the durable App scope of each delivery. Registration is unavailable when the service\'s encryption keyring is not configured.  Registration is installation management rather than work inside an app, so it requires a credential that is *not* associated with an app; a credential bound to one app cannot mint siblings for itself. Names identify apps and are unique, so re-registering an existing name is rejected rather than returning the existing app.
     * Register an app
     */
