...
func TestTurnAdmissionRetryReusesGeneratedIdempotencyKey(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		_, _ = writer.Write([]byte(`{"id":"turn_retry","status":"queued"}`))
	}))
	defer server.Close()
	...
	if turn.ID() != "turn_retry" || turn.IdempotencyKey() == "" {
...
func TestRecoveredTurnUsesExplicitAccessAndDrivesHostTool(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		writer.Header().Set("Content-Type", "application/json")
		switch {
		case request.Method == http.MethodGet && request.URL.Path == "/v1/turns/turn_wait/result" && !settled:
			_, _ = writer.Write([]byte(`{
				"turn":{"id":"turn_wait","status":"waiting","ended_at":null,
				"tool_calls":[{"id":"call_1","name":"lookup_order","mode":"host","status":"pending","arguments":{"order_id":"42"},"updated_at":"2026-08-26T12:00:00Z"}]},
				"messages":[],"output_text":null
			}`))
		case request.Method == http.MethodPost && request.URL.Path == "/v1/turns/turn_wait/tool-results":
			var body struct {
				Results []struct {
			...
			if len(body.Results) != 1 || body.Results[0].ToolCallID != "call_1" {
				t.Fatalf("host results = %#v", body.Results)
			}
			settled = true
			writer.WriteHeader(http.StatusAccepted)
			_, _ = writer.Write([]byte(`{"content_expires_at":null,"conversation_id":null,"results":[],"status":"queued","tool_calls":[],"turn_id":"turn_wait"}`))
		case request.Method == http.MethodGet && request.URL.Path == "/v1/turns/turn_wait/result":
			_, _ = writer.Write([]byte(`{"turn":{"id":"turn_wait","status":"completed","ended_at":"2026-08-26T12:00:01Z"},"messages":[],"output_text":"found"}`))
		default:
			http.NotFound(writer, request)
		}
	}))
	...
	called := false
	turn := client.Turn("turn_wait", TurnAccess{TenantKey: "acme", UserKey: "alice"}).BindTools(Tool{
		Name: "lookup_order",
		Handler: func(_ context.Context, input any, toolContext TurnToolContext) (any, error) {
			called = true
			if toolContext.TurnID != "turn_wait" || toolContext.ToolCallID != "call_1" {
