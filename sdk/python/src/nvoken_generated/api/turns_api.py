...
class TurnsApi:
    ...
    async def stream_turn(
        self,
        turn_id: StrictStr,
        cursor: Annotated[Optional[Annotated[str, Field(min_length=1, strict=True)]], Field(description="Resume position: the `cursor` of the last `transcript.update` you applied, which is also that frame's SSE `id`. It is bound to the scope that issued it. A Turn cursor is accepted by that Turn's stream and by its Conversation's stream; a Conversation cursor only by the Conversation stream. Wins over `Last-Event-ID` when both are sent. ")] = None,
        deltas: Annotated[Optional[StrictBool], Field(description="Whether to send the id-less `message.delta` and `stream.resync` frames. Defaults to true. With `false` the connection carries only saved frames and `connection.closing`; replay, resumption, and settlement are unchanged. ")] = None,
        last_event_id: Annotated[Optional[Annotated[str, Field(min_length=1, strict=True)]], Field(description="The resume position, as `EventSource`-style clients send it on reconnect. Same value and scope rules as `cursor`, which wins when both are present. At most one header, and never blank. ")] = None,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
            Tuple[
        ...
        :param turn_id: (required)
        :type turn_id: str
        :param cursor: Resume position: the `cursor` of the last `transcript.update` you applied, which is also that frame's SSE `id`. It is bound to the scope that issued it. A Turn cursor is accepted by that Turn's stream and by its Conversation's stream; a Conversation cursor only by the Conversation stream. Wins over `Last-Event-ID` when both are sent.
        :type cursor: str
        :param deltas: Whether to send the id-less `message.delta` and `stream.resync` frames. Defaults to true. With `false` the connection carries only saved frames and `connection.closing`; replay, resumption, and settlement are unchanged.
        :type deltas: bool
        :param last_event_id: The resume position, as `EventSource`-style clients send it on reconnect. Same value and scope rules as `cursor`, which wins when both are present. At most one header, and never blank.
