...
class Invocation(BaseModel):
    ...
    definition: Optional[AgentDefinition] = Field(description="The agent definition this turn actually ran with, stored when the turn started and returned exactly as it was. Request headers for remote MCP servers are never stored and never appear here.  Present on `GET /v1/invocations/{id}` and on the result. Null in list items, where `definition_id` identifies it instead. ")
