...
class Invocation(BaseModel):
    ...
    session_id: Optional[Annotated[str, Field(min_length=1, strict=True)]] = Field(description="Conversation identity, or null for a standalone Invocation.")
    ...
    erased_at: Optional[datetime] = Field(default=None, description="Present on minimal fact-backed lineage rows after erasure.")
    ...
    __properties: ClassVar[List[str]] = ["id", "agent_id", "agent_key", "session_id", "content_expires_at", "user_key", "triggered_by", "child_invocation_counts", "definition_id", "definition_revision", "definition", "context", "deduplicated", "status", "stop_reason", "credit_block", "attempt", "error", "usage", "provenance", "structured_output", "structured_output_provenance", "metadata", "limits", "active_execution_ms", "deadline_at", "created_at", "updated_at", "ended_at", "erased_at", "tool_calls"]
