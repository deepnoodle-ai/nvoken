...
class Sampling(BaseModel):
    ...
    Sampling
    """ # noqa: E501
    temperature: Union[Annotated[float, Field(le=1, strict=True, ge=0)], Annotated[int, Field(le=1, strict=True, ge=0)]] = Field(description="Sampling temperature, expressed the same way across providers. Omit `sampling` entirely to keep the provider's own default; an explicit `0` means 0 and is not treated as absent.  If the model is not known to support temperature, the turn is refused rather than run with the setting quietly dropped. ")
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of Sampling from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of Sampling from a dict"""
        if obj is None:
            return None
