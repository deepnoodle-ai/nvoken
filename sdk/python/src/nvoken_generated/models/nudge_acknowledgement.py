...
class NudgeAcknowledgement(BaseModel):
    ...
    """ # noqa: E501
    nudge_id: Annotated[str, Field(min_length=1, strict=True)] = Field(description="Opaque identifier with the public `nudge_` prefix. Treat the body as opaque.")
    status: NudgeStatus
    deduplicated: StrictBool
    after_sequence: StrictInt = Field(description="Transcript position to watch from for the promoted message. It is the Session cursor as of this call for a Nudge that has not been drained, and the position immediately before the promoted message for a Nudge that already has, so reading after it finds the message either way. ")
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of NudgeAcknowledgement from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of NudgeAcknowledgement from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "nudge_id": obj.get("nudge_id"),
            "status": obj.get("status"),
            "deduplicated": obj.get("deduplicated"),
            "after_sequence": obj.get("after_sequence")
        })
        return _obj
