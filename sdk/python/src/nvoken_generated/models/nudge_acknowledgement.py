...
class NudgeAcknowledgement(BaseModel):
    ...
    """ # noqa: E501
    pending_input_id: Annotated[str, Field(strict=True)] = Field(description="UUIDv7 with the public `npin_` prefix.")
    state: PendingInputStatus
    deduped: StrictBool
    after_sequence: StrictInt = Field(description="Transcript position to watch from for the promoted message. It is the Session cursor as of this call for input that has not been drained, and the position immediately before the promoted message for input that already has, so reading after it finds the message either way. ")
    __properties: ClassVar[List[str]] = ["pending_input_id", "state", "deduped", "after_sequence"]

    @field_validator('pending_input_id')
    def pending_input_id_validate_regular_expression(cls, value):
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "pending_input_id": obj.get("pending_input_id"),
            "state": obj.get("state"),
