...
from nvoken_generated.models.invocation_session_options import InvocationSessionOptions
...
class InvocationSession(BaseModel):
    ...
    InvocationSession
    """ # noqa: E501
    mode: StrictStr = Field(description="`new` creates a conversation, `continue` requires an existing conversation selected by `id`, and `continue_or_create` resolves or creates a keyed conversation. ")
    id: Optional[Annotated[str, Field(min_length=1, strict=True)]] = Field(default=None, description="Existing conversation ID. Valid only with `continue`.")
    key: Optional[Annotated[str, Field(min_length=1, strict=True, max_length=255)]] = Field(default=None, description="Caller key for `continue_or_create`.")
    if_active: Optional[StrictStr] = Field(default='reject', description="Policy for an existing conversation's nonterminal turn. Valid only when the mode can continue a Session. ")
    options: Optional[InvocationSessionOptions] = None
    __properties: ClassVar[List[str]] = ["mode", "id", "key", "if_active", "options"]

    @field_validator('mode')
    def mode_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['new', 'continue', 'continue_or_create']):
            raise ValueError("must be one of enum values ('new', 'continue', 'continue_or_create')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of InvocationSession from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of InvocationSession from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "options": InvocationSessionOptions.from_dict(obj["options"]) if obj.get("options") is not None else None
        })
        return _obj
