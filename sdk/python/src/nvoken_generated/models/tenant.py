...
class Tenant(BaseModel):
    ...
    last_invocation_at: Optional[datetime] = Field(description="When this tenant last had a turn admitted, or null if it never has. Read from retained Invocation facts, so Session deletion does not change it. ")
    created_at: datetime = Field(description="When the key was first interned, by any path.")
    __properties: ClassVar[List[str]] = ["id", "tenant_key", "credits", "last_invocation_at", "created_at"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "credits": TenantCredits.from_dict(obj["credits"]) if obj.get("credits") is not None else None,
            "last_invocation_at": obj.get("last_invocation_at"),
