...
BROWSERMEMORYACCESS_ONE_OF_SCHEMAS = ["BrowserNoMemoryAccess", "BrowserUserMemoryAccess"]

class BrowserMemoryAccess(BaseModel):
    """
    BrowserMemoryAccess
    """
    # data type: BrowserNoMemoryAccess
    oneof_schema_1_validator: Optional[BrowserNoMemoryAccess] = None
    # data type: BrowserUserMemoryAccess
    oneof_schema_2_validator: Optional[BrowserUserMemoryAccess] = None
    actual_instance: Optional[Union[BrowserNoMemoryAccess, BrowserUserMemoryAccess]] = None
    one_of_schemas: Set[str] = { "BrowserNoMemoryAccess", "BrowserUserMemoryAccess" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = BrowserMemoryAccess.model_construct()
        error_messages = []
        match = 0
        # validate data type: BrowserNoMemoryAccess
        if not isinstance(v, BrowserNoMemoryAccess):
            error_messages.append(f"Error! Input type `{type(v)}` is not `BrowserNoMemoryAccess`")
        else:
            match += 1
        # validate data type: BrowserUserMemoryAccess
        if not isinstance(v, BrowserUserMemoryAccess):
            error_messages.append(f"Error! Input type `{type(v)}` is not `BrowserUserMemoryAccess`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in BrowserMemoryAccess with oneOf schemas: BrowserNoMemoryAccess, BrowserUserMemoryAccess. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in BrowserMemoryAccess with oneOf schemas: BrowserNoMemoryAccess, BrowserUserMemoryAccess. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into BrowserNoMemoryAccess
        try:
            instance.actual_instance = BrowserNoMemoryAccess.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into BrowserUserMemoryAccess
        try:
            instance.actual_instance = BrowserUserMemoryAccess.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into BrowserMemoryAccess with oneOf schemas: BrowserNoMemoryAccess, BrowserUserMemoryAccess. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into BrowserMemoryAccess with oneOf schemas: BrowserNoMemoryAccess, BrowserUserMemoryAccess. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], BrowserNoMemoryAccess, BrowserUserMemoryAccess]]:
