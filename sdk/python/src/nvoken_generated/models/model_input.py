...
MODELINPUT_ONE_OF_SCHEMAS = ["Model", "str"]

class ModelInput(BaseModel):
    """
    Which model to use. The object form — separate `provider` and `id` — is the real shape. You may also send a single `provider/id` string for convenience; nvoken splits it at the first slash and always returns the object form.
    """
    # data type: Model
    oneof_schema_1_validator: Optional[Model] = None
    # data type: str
    oneof_schema_2_validator: Optional[Annotated[str, Field(strict=True, max_length=511)]] = None
    actual_instance: Optional[Union[Model, str]] = None
    one_of_schemas: Set[str] = { "Model", "str" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = ModelInput.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in ModelInput with oneOf schemas: Model, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in ModelInput with oneOf schemas: Model, str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into ModelInput with oneOf schemas: Model, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ModelInput with oneOf schemas: Model, str. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], Model, str]]:
