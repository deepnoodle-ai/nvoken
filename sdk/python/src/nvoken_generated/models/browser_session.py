...
class BrowserSession(BaseModel):
    ...
    created_at: datetime
    updated_at: datetime
    tool_calls: Optional[List[ToolCallSummary]] = Field(default=None, description="Tool calls of the active Invocation, with their current status. `arguments` and `deadline_at` appear only on eligible named host-mode calls declared by the admitted definition, which are the only ones a browser may answer. ")
    __properties: ClassVar[List[str]] = ["id", "active_invocation_id", "active_invocation_status", "created_at", "updated_at", "tool_calls"]
