...
from nvoken_generated.models.browser_invocation_result import BrowserInvocationResult
from nvoken_generated.models.invocation_result import InvocationResult
from pydantic import StrictStr, Field
from typing import Union, List, Set, Optional, Dict
from typing_extensions import Literal, Self

INVOCATIONRESULTRESPONSE_ONE_OF_SCHEMAS = ["BrowserInvocationResult", "InvocationResult"]

class InvocationResultResponse(BaseModel):
    """
    InvocationResultResponse
    """
    # data type: InvocationResult
    oneof_schema_1_validator: Optional[InvocationResult] = None
    # data type: BrowserInvocationResult
    oneof_schema_2_validator: Optional[BrowserInvocationResult] = None
    actual_instance: Optional[Union[BrowserInvocationResult, InvocationResult]] = None
    one_of_schemas: Set[str] = { "BrowserInvocationResult", "InvocationResult" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = InvocationResultResponse.model_construct()
        error_messages = []
        match = 0
        # validate data type: InvocationResult
        if not isinstance(v, InvocationResult):
            error_messages.append(f"Error! Input type `{type(v)}` is not `InvocationResult`")
        else:
            match += 1
        # validate data type: BrowserInvocationResult
        if not isinstance(v, BrowserInvocationResult):
            error_messages.append(f"Error! Input type `{type(v)}` is not `BrowserInvocationResult`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in InvocationResultResponse with oneOf schemas: BrowserInvocationResult, InvocationResult. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in InvocationResultResponse with oneOf schemas: BrowserInvocationResult, InvocationResult. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into InvocationResult
        try:
            instance.actual_instance = InvocationResult.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into BrowserInvocationResult
        try:
            instance.actual_instance = BrowserInvocationResult.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into InvocationResultResponse with oneOf schemas: BrowserInvocationResult, InvocationResult. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into InvocationResultResponse with oneOf schemas: BrowserInvocationResult, InvocationResult. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], BrowserInvocationResult, InvocationResult]]:
