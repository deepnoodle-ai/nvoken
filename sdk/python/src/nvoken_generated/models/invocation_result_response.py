...
INVOCATIONRESULTRESPONSE_ONE_OF_SCHEMAS = ["ClientInvocationResult", "InvocationResult"]
...
class InvocationResultResponse(BaseModel):
    ...
    # data type: ClientInvocationResult
    oneof_schema_2_validator: Optional[ClientInvocationResult] = None
    actual_instance: Optional[Union[ClientInvocationResult, InvocationResult]] = None
    one_of_schemas: Set[str] = { "ClientInvocationResult", "InvocationResult" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if not isinstance(v, InvocationResult):
            ...
            match += 1
        # validate data type: ClientInvocationResult
        if not isinstance(v, ClientInvocationResult):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ClientInvocationResult`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in InvocationResultResponse with oneOf schemas: ClientInvocationResult, InvocationResult. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in InvocationResultResponse with oneOf schemas: ClientInvocationResult, InvocationResult. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into ClientInvocationResult
        try:
            instance.actual_instance = ClientInvocationResult.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into InvocationResultResponse with oneOf schemas: ClientInvocationResult, InvocationResult. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into InvocationResultResponse with oneOf schemas: ClientInvocationResult, InvocationResult. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ClientInvocationResult, InvocationResult]]:
