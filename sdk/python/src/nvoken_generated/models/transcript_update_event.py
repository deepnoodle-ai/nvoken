...
class TranscriptUpdateEvent(BaseModel):
    """
    The saved frame, and the only one. Messages append by `sequence` and are never sent twice. Changes are an append log keyed by `(invocation_id, revision)`; fold to the highest revision for current state. Within one frame apply messages before changes, so a turn is never marked settled before its final message exists.
    """ # noqa: E501
    type: StrictStr
    ...
    __properties: ClassVar[List[str]] = ["type", "session_id", "content_expires_at", "messages", "invocation_changes", "cursor"]
