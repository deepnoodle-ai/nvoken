...
PROVIDERKEYSELECTION_ONE_OF_SCHEMAS = ["ProviderKeySelectionOneOf", "ProviderKeySelectionOneOf1"]

class ProviderKeySelection(BaseModel):
    """
    Exactly one selection matching the canonical model provider. Omitting `provider_keys` entirely resolves the default chain instead: the app's stored key when one exists, else the self-hosted installation's environment key (config_byok), else platform funding when the installation enables it.
    """
    # data type: ProviderKeySelectionOneOf
    oneof_schema_1_validator: Optional[ProviderKeySelectionOneOf] = None
    # data type: ProviderKeySelectionOneOf1
    oneof_schema_2_validator: Optional[ProviderKeySelectionOneOf1] = None
    actual_instance: Optional[Union[ProviderKeySelectionOneOf, ProviderKeySelectionOneOf1]] = None
    one_of_schemas: Set[str] = { "ProviderKeySelectionOneOf", "ProviderKeySelectionOneOf1" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = ProviderKeySelection.model_construct()
        error_messages = []
        match = 0
        # validate data type: ProviderKeySelectionOneOf
        if not isinstance(v, ProviderKeySelectionOneOf):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ProviderKeySelectionOneOf`")
        else:
            match += 1
        # validate data type: ProviderKeySelectionOneOf1
        if not isinstance(v, ProviderKeySelectionOneOf1):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ProviderKeySelectionOneOf1`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in ProviderKeySelection with oneOf schemas: ProviderKeySelectionOneOf, ProviderKeySelectionOneOf1. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in ProviderKeySelection with oneOf schemas: ProviderKeySelectionOneOf, ProviderKeySelectionOneOf1. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into ProviderKeySelectionOneOf
        try:
            instance.actual_instance = ProviderKeySelectionOneOf.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into ProviderKeySelectionOneOf1
        try:
            instance.actual_instance = ProviderKeySelectionOneOf1.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into ProviderKeySelection with oneOf schemas: ProviderKeySelectionOneOf, ProviderKeySelectionOneOf1. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ProviderKeySelection with oneOf schemas: ProviderKeySelectionOneOf, ProviderKeySelectionOneOf1. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ProviderKeySelectionOneOf, ProviderKeySelectionOneOf1]]:
