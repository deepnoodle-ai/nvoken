...
from nvoken_generated.models.browser_invocation_list import BrowserInvocationList
from nvoken_generated.models.invocation_list import InvocationList
from pydantic import StrictStr, Field
from typing import Union, List, Set, Optional, Dict
from typing_extensions import Literal, Self

INVOCATIONLISTRESPONSE_ONE_OF_SCHEMAS = ["BrowserInvocationList", "InvocationList"]

class InvocationListResponse(BaseModel):
    """
    InvocationListResponse
    """
    # data type: InvocationList
    oneof_schema_1_validator: Optional[InvocationList] = None
    # data type: BrowserInvocationList
    oneof_schema_2_validator: Optional[BrowserInvocationList] = None
    actual_instance: Optional[Union[BrowserInvocationList, InvocationList]] = None
    one_of_schemas: Set[str] = { "BrowserInvocationList", "InvocationList" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = InvocationListResponse.model_construct()
        error_messages = []
        match = 0
        # validate data type: InvocationList
        if not isinstance(v, InvocationList):
            error_messages.append(f"Error! Input type `{type(v)}` is not `InvocationList`")
        else:
            match += 1
        # validate data type: BrowserInvocationList
        if not isinstance(v, BrowserInvocationList):
            error_messages.append(f"Error! Input type `{type(v)}` is not `BrowserInvocationList`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in InvocationListResponse with oneOf schemas: BrowserInvocationList, InvocationList. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in InvocationListResponse with oneOf schemas: BrowserInvocationList, InvocationList. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into InvocationList
        try:
            instance.actual_instance = InvocationList.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into BrowserInvocationList
        try:
            instance.actual_instance = BrowserInvocationList.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into InvocationListResponse with oneOf schemas: BrowserInvocationList, InvocationList. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into InvocationListResponse with oneOf schemas: BrowserInvocationList, InvocationList. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], BrowserInvocationList, InvocationList]]:
