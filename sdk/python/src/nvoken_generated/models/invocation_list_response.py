...
INVOCATIONLISTRESPONSE_ONE_OF_SCHEMAS = ["ClientInvocationList", "InvocationList"]
...
class InvocationListResponse(BaseModel):
    ...
    # data type: ClientInvocationList
    oneof_schema_2_validator: Optional[ClientInvocationList] = None
    actual_instance: Optional[Union[ClientInvocationList, InvocationList]] = None
    one_of_schemas: Set[str] = { "ClientInvocationList", "InvocationList" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if not isinstance(v, InvocationList):
            ...
            match += 1
        # validate data type: ClientInvocationList
        if not isinstance(v, ClientInvocationList):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ClientInvocationList`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in InvocationListResponse with oneOf schemas: ClientInvocationList, InvocationList. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in InvocationListResponse with oneOf schemas: ClientInvocationList, InvocationList. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into ClientInvocationList
        try:
            instance.actual_instance = ClientInvocationList.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into InvocationListResponse with oneOf schemas: ClientInvocationList, InvocationList. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into InvocationListResponse with oneOf schemas: ClientInvocationList, InvocationList. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ClientInvocationList, InvocationList]]:
