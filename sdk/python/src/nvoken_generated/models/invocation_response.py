...
from nvoken_generated.models.browser_invocation import BrowserInvocation
from nvoken_generated.models.invocation import Invocation
from pydantic import StrictStr, Field
from typing import Union, List, Set, Optional, Dict
from typing_extensions import Literal, Self

INVOCATIONRESPONSE_ONE_OF_SCHEMAS = ["BrowserInvocation", "Invocation"]

class InvocationResponse(BaseModel):
    """
    Trusted machine projection or the smaller projection selected by client-token authentication.
    """
    # data type: Invocation
    oneof_schema_1_validator: Optional[Invocation] = None
    # data type: BrowserInvocation
    oneof_schema_2_validator: Optional[BrowserInvocation] = None
    actual_instance: Optional[Union[BrowserInvocation, Invocation]] = None
    one_of_schemas: Set[str] = { "BrowserInvocation", "Invocation" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = InvocationResponse.model_construct()
        error_messages = []
        match = 0
        # validate data type: Invocation
        if not isinstance(v, Invocation):
            error_messages.append(f"Error! Input type `{type(v)}` is not `Invocation`")
        else:
            match += 1
        # validate data type: BrowserInvocation
        if not isinstance(v, BrowserInvocation):
            error_messages.append(f"Error! Input type `{type(v)}` is not `BrowserInvocation`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in InvocationResponse with oneOf schemas: BrowserInvocation, Invocation. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in InvocationResponse with oneOf schemas: BrowserInvocation, Invocation. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into Invocation
        try:
            instance.actual_instance = Invocation.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into BrowserInvocation
        try:
            instance.actual_instance = BrowserInvocation.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into InvocationResponse with oneOf schemas: BrowserInvocation, Invocation. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into InvocationResponse with oneOf schemas: BrowserInvocation, Invocation. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], BrowserInvocation, Invocation]]:
