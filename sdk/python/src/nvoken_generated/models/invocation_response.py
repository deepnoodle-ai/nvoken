...
INVOCATIONRESPONSE_ONE_OF_SCHEMAS = ["ClientInvocation", "Invocation"]
...
class InvocationResponse(BaseModel):
    ...
    # data type: ClientInvocation
    oneof_schema_2_validator: Optional[ClientInvocation] = None
    actual_instance: Optional[Union[ClientInvocation, Invocation]] = None
    one_of_schemas: Set[str] = { "ClientInvocation", "Invocation" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if not isinstance(v, Invocation):
            ...
            match += 1
        # validate data type: ClientInvocation
        if not isinstance(v, ClientInvocation):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ClientInvocation`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in InvocationResponse with oneOf schemas: ClientInvocation, Invocation. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in InvocationResponse with oneOf schemas: ClientInvocation, Invocation. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into ClientInvocation
        try:
            instance.actual_instance = ClientInvocation.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into InvocationResponse with oneOf schemas: ClientInvocation, Invocation. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into InvocationResponse with oneOf schemas: ClientInvocation, Invocation. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ClientInvocation, Invocation]]:
