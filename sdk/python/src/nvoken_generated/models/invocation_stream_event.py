...
class InvocationStreamEvent(BaseModel):
    """
    Typed event from one Invocation stream. Switch on `type`. A minimal consumer prints `output_text.delta` and finishes on `invocation.result`.  New frame types may appear here over time, and existing ones may gain fields. Handle the types you know and ignore the rest.
    """
