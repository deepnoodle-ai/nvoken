...
class ConsoleDeviceAuthorizationGrant(BaseModel):
    """
    ConsoleDeviceAuthorizationGrant
    """ # noqa: E501
    device_code: Annotated[str, Field(min_length=43, strict=True, max_length=43)]
    user_code: Annotated[str, Field(strict=True)]
    expires_in: StrictInt
    interval: StrictInt
    __properties: ClassVar[List[str]] = ["device_code", "user_code", "expires_in", "interval"]
    ...
    def user_code_validate_regular_expression(cls, value):
        ...
        if not re.match(r"^[BCDFGHJKLMNPQRSTVWXZ]{4}-[BCDFGHJKLMNPQRSTVWXZ]{4}$", value):
            raise ValueError(r"must validate the regular expression /^[BCDFGHJKLMNPQRSTVWXZ]{4}-[BCDFGHJKLMNPQRSTVWXZ]{4}$/")
        return value

    @field_validator('expires_in')
    def expires_in_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set([600]):
            raise ValueError("must be one of enum values (600)")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ConsoleDeviceAuthorizationGrant from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ConsoleDeviceAuthorizationGrant from a dict"""
        if obj is None:
            return None
