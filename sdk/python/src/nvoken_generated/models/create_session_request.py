...
class CreateSessionRequest(BaseModel):
    ...
    user_key: Optional[Annotated[str, Field(min_length=1, strict=True, max_length=255)]] = Field(default=None, description="Optional host-owned end-user label recorded on the Session. It is fixed here: every turn in this Session either sends the same one or leaves it out. Required if the Agent's Definition sets `memory.scope: user`, since that makes it the memory partition. ")
