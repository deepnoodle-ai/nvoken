...
from nvoken_generated.models.browser_transcript_stream_event import BrowserTranscriptStreamEvent
from nvoken_generated.models.transcript_stream_event import TranscriptStreamEvent
from pydantic import StrictStr, Field
from typing import Union, List, Set, Optional, Dict
from typing_extensions import Literal, Self

TRANSCRIPTSTREAMRESPONSE_ONE_OF_SCHEMAS = ["BrowserTranscriptStreamEvent", "TranscriptStreamEvent"]

class TranscriptStreamResponse(BaseModel):
    """
    TranscriptStreamResponse
    """
    # data type: TranscriptStreamEvent
    oneof_schema_1_validator: Optional[TranscriptStreamEvent] = None
    # data type: BrowserTranscriptStreamEvent
    oneof_schema_2_validator: Optional[BrowserTranscriptStreamEvent] = None
    actual_instance: Optional[Union[BrowserTranscriptStreamEvent, TranscriptStreamEvent]] = None
    one_of_schemas: Set[str] = { "BrowserTranscriptStreamEvent", "TranscriptStreamEvent" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = TranscriptStreamResponse.model_construct()
        error_messages = []
        match = 0
        # validate data type: TranscriptStreamEvent
        if not isinstance(v, TranscriptStreamEvent):
            error_messages.append(f"Error! Input type `{type(v)}` is not `TranscriptStreamEvent`")
        else:
            match += 1
        # validate data type: BrowserTranscriptStreamEvent
        if not isinstance(v, BrowserTranscriptStreamEvent):
            error_messages.append(f"Error! Input type `{type(v)}` is not `BrowserTranscriptStreamEvent`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in TranscriptStreamResponse with oneOf schemas: BrowserTranscriptStreamEvent, TranscriptStreamEvent. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in TranscriptStreamResponse with oneOf schemas: BrowserTranscriptStreamEvent, TranscriptStreamEvent. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into TranscriptStreamEvent
        try:
            instance.actual_instance = TranscriptStreamEvent.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into BrowserTranscriptStreamEvent
        try:
            instance.actual_instance = BrowserTranscriptStreamEvent.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into TranscriptStreamResponse with oneOf schemas: BrowserTranscriptStreamEvent, TranscriptStreamEvent. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into TranscriptStreamResponse with oneOf schemas: BrowserTranscriptStreamEvent, TranscriptStreamEvent. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], BrowserTranscriptStreamEvent, TranscriptStreamEvent]]:
