...
class CurrentIdentityAuthentication(BaseModel):
    ...
    """ # noqa: E501
    credential_id: Annotated[str, Field(min_length=1, strict=True)] = Field(description="Opaque identifier with the public `cred_` prefix. Treat the body as opaque.")
    app_id: Optional[Annotated[str, Field(min_length=1, strict=True)]] = Field(description="Opaque identifier with the public `app_` prefix. Treat the body as opaque.")
    org_id: Optional[Annotated[str, Field(min_length=1, strict=True)]] = Field(description="Opaque identifier with the public `org_` prefix. Treat the body as opaque.")
    effective_profile: CredentialProfile
    ...
    __properties: ClassVar[List[str]] = ["credential_id", "app_id", "org_id", "effective_profile", "tenant_key", "session_id", "operations", "method", "assurance"]

    @field_validator('method')
    def method_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['api_key', 'issuer_token']):
            raise ValueError("must be one of enum values ('api_key', 'issuer_token')")
        return value
