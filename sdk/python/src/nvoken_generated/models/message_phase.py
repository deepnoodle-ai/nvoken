...
class MessagePhase(str, Enum):
    """
    Tells you which assistant message is the actual answer, so you can render a turn without guessing.  `final_answer` is the message the model ended the turn with — the reply you would show a user. `commentary` is everything else: the model thinking out loud between tool calls, and every message of a turn that was interrupted or stopped early, none of which was ever an answer. A turn with no `final_answer` was left unfinished, and you can show it as such.
    """
