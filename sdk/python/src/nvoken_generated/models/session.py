...
class Session(BaseModel):
    ...
    agent_id: Optional[Annotated[str, Field(min_length=1, strict=True)]] = Field(default=None, description="Null only while a Session created ahead of time has not run a turn yet. The first turn binds the Agent, and after that it never changes. ")
    tenant_key: Optional[StrictStr] = Field(default=None, description="Immutable effective tenant partition reference.")
    session_key: Optional[StrictStr] = None
    user_key: Optional[StrictStr] = Field(default=None, description="Host-owned end-user label, fixed by the request that opened this Session. Every turn carries the same one: send it again and it must match, leave it out and it is inherited. On an Agent whose Definition sets `memory.scope: user` it also selects the memory partition, which is why a turn naming a different end user is refused with `session_user_key_conflict` rather than admitted. ")
    ...
    __properties: ClassVar[List[str]] = ["id", "agent_id", "tenant_key", "session_key", "user_key", "forked_from", "compaction", "retention", "pinned_revision", "expires_at", "metadata", "authorization_context", "active_invocation_id", "active_invocation_status", "credit_block", "context", "usage", "created_at", "updated_at", "tool_calls"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # set to None if authorization_context (nullable) is None
        # and model_fields_set contains the field
        if self.authorization_context is None and "authorization_context" in self.model_fields_set:
            _dict['authorization_context'] = None
