...
class ForkSessionRequest(BaseModel):
    """
    There is no `user_key` here. A fork copies the transcript, including anything a `memory.scope: user` Agent recalled into it, so the child keeps the source Session's end user. Opening a copy of one person's conversation for another is not a fork.
    """ # noqa: E501
    from_message: ForkSessionRequestFromMessage
    session_key: Optional[Annotated[str, Field(min_length=1, strict=True, max_length=255)]] = Field(default=None, description="Optional tenant/Agent-scoped keyed identity for the child.")
    session_options: Optional[ForkSessionOptions] = None
    __properties: ClassVar[List[str]] = ["from_message", "session_key", "session_options"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ForkSessionRequest from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of from_message
        if self.from_message:
            _dict['from_message'] = self.from_message.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ForkSessionRequest from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "from_message": ForkSessionRequestFromMessage.from_dict(obj["from_message"]) if obj.get("from_message") is not None else None,
            "session_key": obj.get("session_key"),
            "session_options": ForkSessionOptions.from_dict(obj["session_options"]) if obj.get("session_options") is not None else None
        })
        return _obj
