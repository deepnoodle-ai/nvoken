...
class SessionContext(BaseModel):
    ...
    SessionContext
    """ # noqa: E501
    estimated_tokens: Annotated[int, Field(strict=True, ge=0)] = Field(description="Roughly how many tokens the model will see on the next turn: the most recent summary, plus the messages that came after it. This is the same number nvoken checks when deciding whether to summarize again. ")
    context_window_tokens: Optional[Annotated[int, Field(strict=True, ge=1)]] = Field(description="Cataloged context window for `model`, or null when the selected model is not in this nvoken installation's catalog. ")
    model: Model = Field(description="Model whose context window applies: the Session compaction model when configured, otherwise the most recent Invocation's primary model. ")
    __properties: ClassVar[List[str]] = ["estimated_tokens", "context_window_tokens", "model"]
