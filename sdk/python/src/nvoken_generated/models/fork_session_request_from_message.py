...
FORKSESSIONREQUESTFROMMESSAGE_ONE_OF_SCHEMAS = ["int", "str"]

class ForkSessionRequestFromMessage(BaseModel):
    """
    Inclusive fork point, by stable message ID or sequence.
    """
    # data type: str
    oneof_schema_1_validator: Optional[Annotated[str, Field(min_length=1, strict=True)]] = Field(default=None, description="Opaque identifier with the public `msg_` prefix. Treat the body as opaque.")
    # data type: int
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = ForkSessionRequestFromMessage.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in ForkSessionRequestFromMessage with oneOf schemas: int, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in ForkSessionRequestFromMessage with oneOf schemas: int, str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into ForkSessionRequestFromMessage with oneOf schemas: int, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ForkSessionRequestFromMessage with oneOf schemas: int, str. Details: " + ", ".join(error_messages))
        else:
            return instance
