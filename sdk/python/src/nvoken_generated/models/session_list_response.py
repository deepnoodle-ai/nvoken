...
from nvoken_generated.models.browser_session_list import BrowserSessionList
from nvoken_generated.models.session_list import SessionList
from pydantic import StrictStr, Field
from typing import Union, List, Set, Optional, Dict
from typing_extensions import Literal, Self

SESSIONLISTRESPONSE_ONE_OF_SCHEMAS = ["BrowserSessionList", "SessionList"]
...
class SessionListResponse(BaseModel):
    ...
    oneof_schema_1_validator: Optional[SessionList] = None
    # data type: BrowserSessionList
    oneof_schema_2_validator: Optional[BrowserSessionList] = None
    actual_instance: Optional[Union[BrowserSessionList, SessionList]] = None
    one_of_schemas: Set[str] = { "BrowserSessionList", "SessionList" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = SessionListResponse.model_construct()
        error_messages = []
        match = 0
        # validate data type: SessionList
        if not isinstance(v, SessionList):
            error_messages.append(f"Error! Input type `{type(v)}` is not `SessionList`")
        else:
            match += 1
        # validate data type: BrowserSessionList
        if not isinstance(v, BrowserSessionList):
            error_messages.append(f"Error! Input type `{type(v)}` is not `BrowserSessionList`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in SessionListResponse with oneOf schemas: BrowserSessionList, SessionList. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in SessionListResponse with oneOf schemas: BrowserSessionList, SessionList. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into SessionList
        try:
            instance.actual_instance = SessionList.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into BrowserSessionList
        try:
            instance.actual_instance = BrowserSessionList.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into SessionListResponse with oneOf schemas: BrowserSessionList, SessionList. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into SessionListResponse with oneOf schemas: BrowserSessionList, SessionList. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], BrowserSessionList, SessionList]]:
