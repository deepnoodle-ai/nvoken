...
INVOCATIONINPUT_ONE_OF_SCHEMAS = ["List[InputBlock]", "str"]

class InvocationInput(BaseModel):
    """
    A plain string is shorthand for one text block; an array is ordered multi-block input mixing text, images, and documents. Both normalize to the same stored message and are therefore equal for idempotency. At most 8 blocks may carry media, and their decoded payloads must total at most 16777216 bytes.
    """
    # data type: str
    oneof_schema_1_validator: Optional[Annotated[str, Field(min_length=1, strict=True)]] = None
    # data type: List[InputBlock]
    oneof_schema_2_validator: Optional[Annotated[List[InputBlock], Field(min_length=1, max_length=64)]] = None
    actual_instance: Optional[Union[List[InputBlock], str]] = None
    one_of_schemas: Set[str] = { "List[InputBlock]", "str" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        # validate data type: List[InputBlock]
        try:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in InvocationInput with oneOf schemas: List[InputBlock], str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in InvocationInput with oneOf schemas: List[InputBlock], str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        try:
            ...
            error_messages.append(str(e))
        # deserialize data into List[InputBlock]
        try:
            # validation
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into InvocationInput with oneOf schemas: List[InputBlock], str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into InvocationInput with oneOf schemas: List[InputBlock], str. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], List[InputBlock], str]]:
