...
class ToolCallSummary(BaseModel):
    """
    One tool call as a stream sees it: which call, what state it is in, and when it reached that state. The `id` is the `id` of the `tool_use` block that opened it, so a client can show a call as failed or still running without waiting for the message that carries its result.  Names, modes, attempts, and delivery detail live on the ToolCall resource at `GET /v1/invocations/{invocation_id}/tool-calls`.
    """ # noqa: E501
