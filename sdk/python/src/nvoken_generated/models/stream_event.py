...
STREAMEVENT_ONE_OF_SCHEMAS = ["ConnectionClosingEvent", "MessageDeltaEvent", "StreamResyncEvent", "TranscriptUpdateEvent"]

class StreamEvent(BaseModel):
    """
    The JSON value carried by one SSE `data:` field, on the Turn and the Conversation stream alike. Switch on `type`. Saved `transcript.update` frames carry the resume cursor as both payload data and SSE `id`; preview and control frames never carry IDs.  New frame types may appear here over time, and existing ones may gain fields. Handle the types you know and ignore the rest.
    """
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = StreamEvent.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in StreamEvent with oneOf schemas: ConnectionClosingEvent, MessageDeltaEvent, StreamResyncEvent, TranscriptUpdateEvent. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in StreamEvent with oneOf schemas: ConnectionClosingEvent, MessageDeltaEvent, StreamResyncEvent, TranscriptUpdateEvent. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into StreamEvent with oneOf schemas: ConnectionClosingEvent, MessageDeltaEvent, StreamResyncEvent, TranscriptUpdateEvent. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into StreamEvent with oneOf schemas: ConnectionClosingEvent, MessageDeltaEvent, StreamResyncEvent, TranscriptUpdateEvent. Details: " + ", ".join(error_messages))
        else:
            return instance
