...
TRANSCRIPTSTREAMEVENT_ONE_OF_SCHEMAS = ["OutputTextDeltaEvent", "StreamEndEvent", "StreamResyncEvent", "ThinkingDeltaEvent", "TranscriptUpdateEvent"]
...
class TranscriptStreamEvent(BaseModel):
    ...
    actual_instance: Optional[Union[OutputTextDeltaEvent, StreamEndEvent, StreamResyncEvent, ThinkingDeltaEvent, TranscriptUpdateEvent]] = None
    one_of_schemas: Set[str] = { "OutputTextDeltaEvent", "StreamEndEvent", "StreamResyncEvent", "ThinkingDeltaEvent", "TranscriptUpdateEvent" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in TranscriptStreamEvent with oneOf schemas: OutputTextDeltaEvent, StreamEndEvent, StreamResyncEvent, ThinkingDeltaEvent, TranscriptUpdateEvent. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in TranscriptStreamEvent with oneOf schemas: OutputTextDeltaEvent, StreamEndEvent, StreamResyncEvent, ThinkingDeltaEvent, TranscriptUpdateEvent. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into TranscriptStreamEvent with oneOf schemas: OutputTextDeltaEvent, StreamEndEvent, StreamResyncEvent, ThinkingDeltaEvent, TranscriptUpdateEvent. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into TranscriptStreamEvent with oneOf schemas: OutputTextDeltaEvent, StreamEndEvent, StreamResyncEvent, ThinkingDeltaEvent, TranscriptUpdateEvent. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], OutputTextDeltaEvent, StreamEndEvent, StreamResyncEvent, ThinkingDeltaEvent, TranscriptUpdateEvent]]:
