...
class ReminderBlock(BaseModel):
    ...
    """ # noqa: E501
    type: StrictStr
    name: Annotated[str, Field(min_length=5, strict=True, max_length=68)] = Field(description="Caller name in nvoken's reserved application namespace.")
    tier: StrictStr
    content: Annotated[str, Field(min_length=1, strict=True)]
    __properties: ClassVar[List[str]] = ["type", "name", "tier", "content"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['reminder']):
            raise ValueError("must be one of enum values ('reminder')")
        return value
    ...
    def name_validate_regular_expression(cls, value):
        ...
        if not re.match(r"^app-[a-z][a-z0-9-]*$", value):
            raise ValueError(r"must validate the regular expression /^app-[a-z][a-z0-9-]*$/")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ReminderBlock from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ReminderBlock from a dict"""
        if obj is None:
            return None
