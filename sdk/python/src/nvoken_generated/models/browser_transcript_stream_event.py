...
from nvoken_generated.models.browser_transcript_update_event import BrowserTranscriptUpdateEvent
...
BROWSERTRANSCRIPTSTREAMEVENT_ONE_OF_SCHEMAS = ["BrowserTranscriptUpdateEvent", "OutputTextDeltaEvent", "StreamEndEvent", "StreamResyncEvent", "ThinkingDeltaEvent"]

class BrowserTranscriptStreamEvent(BaseModel):
    """
    Browser stream event. Switch on `type`. Host-only transcript fields are never emitted. New frame types may appear here over time, and existing ones may gain fields.
    """
    # data type: BrowserTranscriptUpdateEvent
    oneof_schema_1_validator: Optional[BrowserTranscriptUpdateEvent] = None
    ...
    oneof_schema_4_validator: Optional[StreamResyncEvent] = None
    # data type: StreamEndEvent
    oneof_schema_5_validator: Optional[StreamEndEvent] = None
    actual_instance: Optional[Union[BrowserTranscriptUpdateEvent, OutputTextDeltaEvent, StreamEndEvent, StreamResyncEvent, ThinkingDeltaEvent]] = None
    one_of_schemas: Set[str] = { "BrowserTranscriptUpdateEvent", "OutputTextDeltaEvent", "StreamEndEvent", "StreamResyncEvent", "ThinkingDeltaEvent" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = BrowserTranscriptStreamEvent.model_construct()
        error_messages = []
        match = 0
        # validate data type: BrowserTranscriptUpdateEvent
        if not isinstance(v, BrowserTranscriptUpdateEvent):
            error_messages.append(f"Error! Input type `{type(v)}` is not `BrowserTranscriptUpdateEvent`")
        else:
            match += 1
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in BrowserTranscriptStreamEvent with oneOf schemas: BrowserTranscriptUpdateEvent, OutputTextDeltaEvent, StreamEndEvent, StreamResyncEvent, ThinkingDeltaEvent. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in BrowserTranscriptStreamEvent with oneOf schemas: BrowserTranscriptUpdateEvent, OutputTextDeltaEvent, StreamEndEvent, StreamResyncEvent, ThinkingDeltaEvent. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into BrowserTranscriptUpdateEvent
        try:
            instance.actual_instance = BrowserTranscriptUpdateEvent.from_json(json_str)
            match += 1
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into BrowserTranscriptStreamEvent with oneOf schemas: BrowserTranscriptUpdateEvent, OutputTextDeltaEvent, StreamEndEvent, StreamResyncEvent, ThinkingDeltaEvent. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into BrowserTranscriptStreamEvent with oneOf schemas: BrowserTranscriptUpdateEvent, OutputTextDeltaEvent, StreamEndEvent, StreamResyncEvent, ThinkingDeltaEvent. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], BrowserTranscriptUpdateEvent, OutputTextDeltaEvent, StreamEndEvent, StreamResyncEvent, ThinkingDeltaEvent]]:
