...
class ThinkingDeltaEvent(BaseModel):
    """
    A live preview of the model's reasoning, carried on machine and browser streams alike. What an end user sees is your application's decision, made in your application.  Reasoning is watchable and not stored: no content block carries it, and no read returns it. Watch the stream if you want to see the model think. A turn that runs under explicit `reasoning` controls emits no reasoning previews at all, to either audience.
    """ # noqa: E501
    type: StrictStr
    ...
    message_id: Optional[Annotated[str, Field(min_length=1, strict=True)]] = Field(default=None, description="The identifier the saved assistant message will carry when this iteration lands, with the same `msg_` prefix every message ID has. Key your preview by it and the handoff becomes an update to a row that already has its permanent identity, rather than one row disappearing and another taking its place.  Optional: a runtime that has not allocated the identifier yet leaves it out. It is stable within one attempt. A retried attempt allocates a new one, which is harmless, because a higher `attempt` already voids every earlier preview. ")
    thinking: Annotated[str, Field(min_length=1, strict=True)]
    emitted_at: datetime
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["type", "session_id", "invocation_id", "attempt", "iteration", "content_index", "message_id", "thinking", "emitted_at"]
