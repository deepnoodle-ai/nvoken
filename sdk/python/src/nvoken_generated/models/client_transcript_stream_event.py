...
CLIENTTRANSCRIPTSTREAMEVENT_ONE_OF_SCHEMAS = ["ClientTranscriptUpdate", "OutputTextDeltaEvent", "StreamEndEvent", "StreamResyncEvent"]

class ClientTranscriptStreamEvent(BaseModel):
    """
    Browser stream event. Thinking previews and host-only transcript fields are never emitted.
    """
    # data type: ClientTranscriptUpdate
    oneof_schema_1_validator: Optional[ClientTranscriptUpdate] = None
    ...
    actual_instance: Optional[Union[ClientTranscriptUpdate, OutputTextDeltaEvent, StreamEndEvent, StreamResyncEvent]] = None
    one_of_schemas: Set[str] = { "ClientTranscriptUpdate", "OutputTextDeltaEvent", "StreamEndEvent", "StreamResyncEvent" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = ClientTranscriptStreamEvent.model_construct()
        error_messages = []
        match = 0
        # validate data type: ClientTranscriptUpdate
        if not isinstance(v, ClientTranscriptUpdate):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ClientTranscriptUpdate`")
        else:
            match += 1
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in ClientTranscriptStreamEvent with oneOf schemas: ClientTranscriptUpdate, OutputTextDeltaEvent, StreamEndEvent, StreamResyncEvent. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in ClientTranscriptStreamEvent with oneOf schemas: ClientTranscriptUpdate, OutputTextDeltaEvent, StreamEndEvent, StreamResyncEvent. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into ClientTranscriptUpdate
        try:
            instance.actual_instance = ClientTranscriptUpdate.from_json(json_str)
            match += 1
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into ClientTranscriptStreamEvent with oneOf schemas: ClientTranscriptUpdate, OutputTextDeltaEvent, StreamEndEvent, StreamResyncEvent. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ClientTranscriptStreamEvent with oneOf schemas: ClientTranscriptUpdate, OutputTextDeltaEvent, StreamEndEvent, StreamResyncEvent. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ClientTranscriptUpdate, OutputTextDeltaEvent, StreamEndEvent, StreamResyncEvent]]:
