...
from nvoken_generated.models.browser_current_identity import BrowserCurrentIdentity
from nvoken_generated.models.current_identity import CurrentIdentity
from pydantic import StrictStr, Field
from typing import Union, List, Set, Optional, Dict
from typing_extensions import Literal, Self

CURRENTIDENTITYRESPONSE_ONE_OF_SCHEMAS = ["BrowserCurrentIdentity", "CurrentIdentity"]

class CurrentIdentityResponse(BaseModel):
    """
    CurrentIdentityResponse
    """
    # data type: CurrentIdentity
    oneof_schema_1_validator: Optional[CurrentIdentity] = None
    # data type: BrowserCurrentIdentity
    oneof_schema_2_validator: Optional[BrowserCurrentIdentity] = None
    actual_instance: Optional[Union[BrowserCurrentIdentity, CurrentIdentity]] = None
    one_of_schemas: Set[str] = { "BrowserCurrentIdentity", "CurrentIdentity" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = CurrentIdentityResponse.model_construct()
        error_messages = []
        match = 0
        # validate data type: CurrentIdentity
        if not isinstance(v, CurrentIdentity):
            error_messages.append(f"Error! Input type `{type(v)}` is not `CurrentIdentity`")
        else:
            match += 1
        # validate data type: BrowserCurrentIdentity
        if not isinstance(v, BrowserCurrentIdentity):
            error_messages.append(f"Error! Input type `{type(v)}` is not `BrowserCurrentIdentity`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in CurrentIdentityResponse with oneOf schemas: BrowserCurrentIdentity, CurrentIdentity. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in CurrentIdentityResponse with oneOf schemas: BrowserCurrentIdentity, CurrentIdentity. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into CurrentIdentity
        try:
            instance.actual_instance = CurrentIdentity.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into BrowserCurrentIdentity
        try:
            instance.actual_instance = BrowserCurrentIdentity.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into CurrentIdentityResponse with oneOf schemas: BrowserCurrentIdentity, CurrentIdentity. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into CurrentIdentityResponse with oneOf schemas: BrowserCurrentIdentity, CurrentIdentity. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], BrowserCurrentIdentity, CurrentIdentity]]:
