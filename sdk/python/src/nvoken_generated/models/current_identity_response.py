...
CURRENTIDENTITYRESPONSE_ONE_OF_SCHEMAS = ["ClientCurrentIdentity", "CurrentIdentity"]
...
class CurrentIdentityResponse(BaseModel):
    ...
    # data type: ClientCurrentIdentity
    oneof_schema_2_validator: Optional[ClientCurrentIdentity] = None
    actual_instance: Optional[Union[ClientCurrentIdentity, CurrentIdentity]] = None
    one_of_schemas: Set[str] = { "ClientCurrentIdentity", "CurrentIdentity" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if not isinstance(v, CurrentIdentity):
            ...
            match += 1
        # validate data type: ClientCurrentIdentity
        if not isinstance(v, ClientCurrentIdentity):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ClientCurrentIdentity`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in CurrentIdentityResponse with oneOf schemas: ClientCurrentIdentity, CurrentIdentity. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in CurrentIdentityResponse with oneOf schemas: ClientCurrentIdentity, CurrentIdentity. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into ClientCurrentIdentity
        try:
            instance.actual_instance = ClientCurrentIdentity.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into CurrentIdentityResponse with oneOf schemas: ClientCurrentIdentity, CurrentIdentity. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into CurrentIdentityResponse with oneOf schemas: ClientCurrentIdentity, CurrentIdentity. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ClientCurrentIdentity, CurrentIdentity]]:
