...
AGENTSELECTOR_ONE_OF_SCHEMAS = ["AgentSelectorByID", "AgentSelectorByKey"]

class AgentSelector(BaseModel):
    """
    AgentSelector
    """
    # data type: AgentSelectorByID
    oneof_schema_1_validator: Optional[AgentSelectorByID] = None
    # data type: AgentSelectorByKey
    oneof_schema_2_validator: Optional[AgentSelectorByKey] = None
    actual_instance: Optional[Union[AgentSelectorByID, AgentSelectorByKey]] = None
    one_of_schemas: Set[str] = { "AgentSelectorByID", "AgentSelectorByKey" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = AgentSelector.model_construct()
        error_messages = []
        match = 0
        # validate data type: AgentSelectorByID
        if not isinstance(v, AgentSelectorByID):
            error_messages.append(f"Error! Input type `{type(v)}` is not `AgentSelectorByID`")
        else:
            match += 1
        # validate data type: AgentSelectorByKey
        if not isinstance(v, AgentSelectorByKey):
            error_messages.append(f"Error! Input type `{type(v)}` is not `AgentSelectorByKey`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in AgentSelector with oneOf schemas: AgentSelectorByID, AgentSelectorByKey. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in AgentSelector with oneOf schemas: AgentSelectorByID, AgentSelectorByKey. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into AgentSelectorByID
        try:
            instance.actual_instance = AgentSelectorByID.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into AgentSelectorByKey
        try:
            instance.actual_instance = AgentSelectorByKey.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into AgentSelector with oneOf schemas: AgentSelectorByID, AgentSelectorByKey. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into AgentSelector with oneOf schemas: AgentSelectorByID, AgentSelectorByKey. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], AgentSelectorByID, AgentSelectorByKey]]:
