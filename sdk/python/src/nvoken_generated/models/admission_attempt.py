...
class AdmissionAttempt(BaseModel):
    ...
    invocation_id: Optional[Annotated[str, Field(min_length=1, strict=True)]] = Field(description="The admitted Invocation, or null for a refusal. It carries no foreign key: the record outlives Session erasure, so a deleted transcript never erases the evidence that the request happened. ")
    deduplicated: StrictBool = Field(description="Whether the request replayed an existing Invocation.")
    ...
    __properties: ClassVar[List[str]] = ["id", "attempted_at", "outcome", "error_code", "http_status", "tenant_key", "user_key", "agent_key", "provider", "model", "credential_family_id", "authentication_method", "request_id", "invocation_id", "deduplicated", "details"]
