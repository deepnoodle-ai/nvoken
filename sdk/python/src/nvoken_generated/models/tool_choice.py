...
class ToolChoice(BaseModel):
    """
    Portable Invocation tool selection. auto preserves normal selection; none disables tools for the Invocation; required and named apply only to the first durable model iteration, then return to auto. name is required only for named mode. The final bounded iteration always disables tools.
    """ # noqa: E501
    mode: ModelToolChoiceMode
    name: Optional[Annotated[str, Field(min_length=1, strict=True, max_length=64)]] = None
    __properties: ClassVar[List[str]] = ["mode", "name"]
