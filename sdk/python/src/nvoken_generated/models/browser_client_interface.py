...
class BrowserClientInterface(BaseModel):
    """
    One immutable AgentRevision-specific authorization object. Omission means the AgentRevision is not client-token-capable; an explicit empty object opts in with no client-authored context or tools. Each set is sorted before a revision snapshot is retained, so order alone cannot advance a resource revision.  This object grants authorship and settlement only, never selective read visibility. All public transcript content in a browser-reachable Conversation, including operator-tier recorded context, must be treated as client-visible. No lifetime, host_only, wildcard, generic tool-policy, tier-switch, or disclosure field exists here.
    """ # noqa: E501
    ...
    tool_names: Optional[Annotated[List[Annotated[str, Field(min_length=1, strict=True, max_length=64)]], Field(max_length=32)]] = Field(default=None, description="Exact subset of this AgentRevision's declared host-mode tools whose parked calls a browser client principal may see and settle. ")
