...
class BrowserClientInterface(BaseModel):
    """
    One immutable definition-specific authorization object. Omission means the definition is not client-token-capable; an explicit empty object opts in with no client-authored context or tools. Each set is sorted before a revision snapshot is retained, so order alone cannot advance a resource revision.  This object grants authorship and settlement only, never selective read visibility. All public transcript content in a browser-reachable Session, including operator-tier recorded context, must be treated as client-visible. No lifetime, host_only, wildcard, generic tool-policy, tier-switch, or disclosure field exists here.
    """ # noqa: E501
    context_names: Optional[Annotated[List[Annotated[str, Field(min_length=1, strict=True, max_length=64)]], Field(max_length=16)]] = Field(default=None, description="Exact recorded-context names a browser client principal may append or supersede. Every item is contextual tier; clients never author operator-tier context. ")
    tool_names: Optional[Annotated[List[Annotated[str, Field(min_length=1, strict=True, max_length=64)]], Field(max_length=32)]] = Field(default=None, description="Exact subset of this definition's declared host-mode tools whose parked calls a browser client principal may see and settle. ")
    __properties: ClassVar[List[str]] = ["context_names", "tool_names"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of BrowserClientInterface from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of BrowserClientInterface from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "context_names": obj.get("context_names"),
