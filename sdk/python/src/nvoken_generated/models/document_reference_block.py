...
class DocumentReferenceBlock(BaseModel):
    """
    A stored input document, described rather than sent back. You get its media type, size, and checksum, but not the bytes.
    """ # noqa: E501
    type: StrictStr
    media_type: StrictStr
    title: Optional[StrictStr] = Field(default=None, description="Present only when the caller supplied a title.")
    bytes: Annotated[int, Field(le=16777216, strict=True, ge=0)] = Field(description="Decoded document byte count; never the document payload.")
    digest: Annotated[str, Field(strict=True)] = Field(description="Digest of the decoded document bytes.")
    url: Optional[StrictStr] = Field(default=None, description="Original URL when the document arrived by URL.")
    __properties: ClassVar[List[str]] = ["type", "media_type", "title", "bytes", "digest", "url"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of DocumentReferenceBlock from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of DocumentReferenceBlock from a dict"""
        if obj is None:
            return None
