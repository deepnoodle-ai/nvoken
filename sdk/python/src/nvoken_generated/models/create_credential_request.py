...
class CreateCredentialRequest(BaseModel):
    ...
    app_id: Optional[Annotated[str, Field(min_length=1, strict=True)]] = Field(default=None, description="Target App for an Org or installation Operator provisioning an App credential. Org callers may name only Apps contained by their Org. App callers may omit this field or repeat their own App ID. ")
    org_id: Optional[Annotated[str, Field(min_length=1, strict=True)]] = Field(default=None, description="Target Org for an installation Operator provisioning an Org credential. Installation issuer tokens require `admin: true`. Mutually exclusive with `app_id`. Org and App callers cannot mint Org credentials. ")
    ...
    __properties: ClassVar[List[str]] = ["name", "profile", "app_id", "org_id", "tenant_key", "session_id", "operations", "expires_at"]
