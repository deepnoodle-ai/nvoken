...
class MintAppSigningKeyRequest(BaseModel):
    """
    MintAppSigningKeyRequest
    """ # noqa: E501
    purpose: AppSigningKeyPurpose
    activate: Optional[StrictBool] = Field(default=False, description="Sign with the new version immediately, collapsing the rotation to one call. Correct only when no working verifier is left to protect — recovering a lost secret. Leaving it false is what makes an ordinary rotation cause zero failed verifications. ")
    __properties: ClassVar[List[str]] = ["purpose", "activate"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of MintAppSigningKeyRequest from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of MintAppSigningKeyRequest from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "activate": obj.get("activate") if obj.get("activate") is not None else False
        })
        return _obj
