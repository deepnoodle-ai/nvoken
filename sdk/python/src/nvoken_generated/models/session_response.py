...
from nvoken_generated.models.browser_session import BrowserSession
from nvoken_generated.models.session import Session
from pydantic import StrictStr, Field
from typing import Union, List, Set, Optional, Dict
from typing_extensions import Literal, Self

SESSIONRESPONSE_ONE_OF_SCHEMAS = ["BrowserSession", "Session"]
...
class SessionResponse(BaseModel):
    ...
    # data type: BrowserSession
    oneof_schema_2_validator: Optional[BrowserSession] = None
    actual_instance: Optional[Union[BrowserSession, Session]] = None
    one_of_schemas: Set[str] = { "BrowserSession", "Session" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = SessionResponse.model_construct()
        error_messages = []
        match = 0
        ...
        if not isinstance(v, Session):
            ...
            match += 1
        # validate data type: BrowserSession
        if not isinstance(v, BrowserSession):
            error_messages.append(f"Error! Input type `{type(v)}` is not `BrowserSession`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in SessionResponse with oneOf schemas: BrowserSession, Session. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in SessionResponse with oneOf schemas: BrowserSession, Session. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into BrowserSession
        try:
            instance.actual_instance = BrowserSession.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into SessionResponse with oneOf schemas: BrowserSession, Session. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into SessionResponse with oneOf schemas: BrowserSession, Session. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], BrowserSession, Session]]:
