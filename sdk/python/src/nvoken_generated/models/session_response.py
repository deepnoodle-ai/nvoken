...
SESSIONRESPONSE_ONE_OF_SCHEMAS = ["ClientSession", "Session"]
...
class SessionResponse(BaseModel):
    ...
    # data type: ClientSession
    oneof_schema_2_validator: Optional[ClientSession] = None
    actual_instance: Optional[Union[ClientSession, Session]] = None
    one_of_schemas: Set[str] = { "ClientSession", "Session" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if not isinstance(v, Session):
            ...
            match += 1
        # validate data type: ClientSession
        if not isinstance(v, ClientSession):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ClientSession`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in SessionResponse with oneOf schemas: ClientSession, Session. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in SessionResponse with oneOf schemas: ClientSession, Session. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into ClientSession
        try:
            instance.actual_instance = ClientSession.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into SessionResponse with oneOf schemas: ClientSession, Session. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into SessionResponse with oneOf schemas: ClientSession, Session. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ClientSession, Session]]:
