...
class ProviderTool(BaseModel):
    """
    Turns on a tool the model provider runs itself, inside the model call — web search, for example.  This behaves differently from your own tools in two ways: the turn never stops to wait for it, and nvoken never executes it. The provider does, and charges it to the same provider key the model is running on.  That is deliberate. A search tool operated by nvoken would mean nvoken holding a vendor relationship with a search provider and answering \"who pays per query\" for every installation. Letting the provider run it keeps one key and one bill.
    """ # noqa: E501
    type: StrictStr
    web_search: WebSearchTool
    __properties: ClassVar[List[str]] = ["type", "web_search"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ProviderTool from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ProviderTool from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "web_search": WebSearchTool.from_dict(obj["web_search"]) if obj.get("web_search") is not None else None
        })
        return _obj
