...
SEEDMESSAGECONTENT_ONE_OF_SCHEMAS = ["List[TextInputBlock]", "str"]

class SeedMessageContent(BaseModel):
    """
    Text-only canonical history. A string is shorthand for one text block; both forms normalize to the same stored content-block array.
    """
    # data type: str
    oneof_schema_1_validator: Optional[Annotated[str, Field(min_length=1, strict=True)]] = None
    # data type: List[TextInputBlock]
    oneof_schema_2_validator: Optional[Annotated[List[TextInputBlock], Field(min_length=1, max_length=64)]] = None
    actual_instance: Optional[Union[List[TextInputBlock], str]] = None
    one_of_schemas: Set[str] = { "List[TextInputBlock]", "str" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = SeedMessageContent.model_construct()
        error_messages = []
        match = 0
        ...
        # validate data type: List[TextInputBlock]
        try:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in SeedMessageContent with oneOf schemas: List[TextInputBlock], str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in SeedMessageContent with oneOf schemas: List[TextInputBlock], str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        try:
            ...
            error_messages.append(str(e))
        # deserialize data into List[TextInputBlock]
        try:
            # validation
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into SeedMessageContent with oneOf schemas: List[TextInputBlock], str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into SeedMessageContent with oneOf schemas: List[TextInputBlock], str. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], List[TextInputBlock], str]]:
