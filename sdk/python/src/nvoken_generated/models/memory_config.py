...
class MemoryConfig(BaseModel):
    """
    Opts this Agent Definition into durable memory and automatically attaches nvoken_memory_remember, nvoken_memory_recall, and nvoken_memory_forget. Omission attaches no memory capability.
    """ # noqa: E501
    scope: Optional[StrictStr] = Field(default='tenant', description="User scope requires user_key on every admitted Invocation.")
    context: Optional[MemoryContextConfig] = None
    __properties: ClassVar[List[str]] = ["scope", "context"]
    ...
    def scope_validate_enum(cls, value):
        ...
        if value not in set(['tenant', 'user']):
            raise ValueError("must be one of enum values ('tenant', 'user')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "scope": obj.get("scope") if obj.get("scope") is not None else 'tenant',
            "context": MemoryContextConfig.from_dict(obj["context"]) if obj.get("context") is not None else None
        })
        return _obj
