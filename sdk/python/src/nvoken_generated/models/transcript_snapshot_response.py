...
TRANSCRIPTSNAPSHOTRESPONSE_ONE_OF_SCHEMAS = ["ClientTranscriptSnapshot", "TranscriptSnapshot"]
...
class TranscriptSnapshotResponse(BaseModel):
    ...
    # data type: ClientTranscriptSnapshot
    oneof_schema_2_validator: Optional[ClientTranscriptSnapshot] = None
    actual_instance: Optional[Union[ClientTranscriptSnapshot, TranscriptSnapshot]] = None
    one_of_schemas: Set[str] = { "ClientTranscriptSnapshot", "TranscriptSnapshot" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if not isinstance(v, TranscriptSnapshot):
            ...
            match += 1
        # validate data type: ClientTranscriptSnapshot
        if not isinstance(v, ClientTranscriptSnapshot):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ClientTranscriptSnapshot`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in TranscriptSnapshotResponse with oneOf schemas: ClientTranscriptSnapshot, TranscriptSnapshot. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in TranscriptSnapshotResponse with oneOf schemas: ClientTranscriptSnapshot, TranscriptSnapshot. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into ClientTranscriptSnapshot
        try:
            instance.actual_instance = ClientTranscriptSnapshot.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into TranscriptSnapshotResponse with oneOf schemas: ClientTranscriptSnapshot, TranscriptSnapshot. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into TranscriptSnapshotResponse with oneOf schemas: ClientTranscriptSnapshot, TranscriptSnapshot. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ClientTranscriptSnapshot, TranscriptSnapshot]]:
