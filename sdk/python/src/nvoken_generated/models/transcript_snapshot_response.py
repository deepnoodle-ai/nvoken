...
from nvoken_generated.models.browser_transcript_snapshot import BrowserTranscriptSnapshot
from nvoken_generated.models.transcript_snapshot import TranscriptSnapshot
from pydantic import StrictStr, Field
from typing import Union, List, Set, Optional, Dict
from typing_extensions import Literal, Self

TRANSCRIPTSNAPSHOTRESPONSE_ONE_OF_SCHEMAS = ["BrowserTranscriptSnapshot", "TranscriptSnapshot"]
...
class TranscriptSnapshotResponse(BaseModel):
    ...
    # data type: BrowserTranscriptSnapshot
    oneof_schema_2_validator: Optional[BrowserTranscriptSnapshot] = None
    actual_instance: Optional[Union[BrowserTranscriptSnapshot, TranscriptSnapshot]] = None
    one_of_schemas: Set[str] = { "BrowserTranscriptSnapshot", "TranscriptSnapshot" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = TranscriptSnapshotResponse.model_construct()
        error_messages = []
        match = 0
        ...
        if not isinstance(v, TranscriptSnapshot):
            ...
            match += 1
        # validate data type: BrowserTranscriptSnapshot
        if not isinstance(v, BrowserTranscriptSnapshot):
            error_messages.append(f"Error! Input type `{type(v)}` is not `BrowserTranscriptSnapshot`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in TranscriptSnapshotResponse with oneOf schemas: BrowserTranscriptSnapshot, TranscriptSnapshot. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in TranscriptSnapshotResponse with oneOf schemas: BrowserTranscriptSnapshot, TranscriptSnapshot. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into BrowserTranscriptSnapshot
        try:
            instance.actual_instance = BrowserTranscriptSnapshot.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into TranscriptSnapshotResponse with oneOf schemas: BrowserTranscriptSnapshot, TranscriptSnapshot. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into TranscriptSnapshotResponse with oneOf schemas: BrowserTranscriptSnapshot, TranscriptSnapshot. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], BrowserTranscriptSnapshot, TranscriptSnapshot]]:
