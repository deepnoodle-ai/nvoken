...
class SessionOptions(BaseModel):
    """
    Settings stored on the Session. Every field is optional, but send at least one. All of them are assertions rather than descriptions: a Session's title and other descriptive data live in `metadata`, which is written only by `PATCH /v1/sessions/{session_id}`.  On a new Session these are saved. When you reach an existing Session, by ID or by key, they are checked rather than applied: matching values are accepted, and a different value returns `session_options_conflict` with `details.conflicting_paths` naming exactly what disagreed. That includes `pinned_revision`, which is immutable, and `authorization_context`, which is never installed on a Session created without one. Compaction is the single exception, and only in one direction: a Session with no policy adopts the one you send, because the policy is validated against a turn's model and so needs a turn to install it.
    """ # noqa: E501
    ...
    authorization_context: Optional[Dict[str, Annotated[str, Field(strict=True, max_length=512)]]] = Field(default=None, description="What this Session is bound to, in your own terms. If your authorization boundary is finer than a tenant — a board, a workspace, a document — this is where you say so, and nvoken carries it to your callback receiver inside the signed body so authorizing a delivery costs no round trip.  Four properties, all guaranteed rather than incidental:  - **Written at creation only.** Set by the request that brings the   Session into existence, whether that is `POST /v1/sessions`, a fork,   or the admission that creates it. There is no patch path and there   will not be one. A later turn that sends a different value is refused   with `session_options_conflict`; one that sends a value on a Session   created without one is refused rather than installing it. - **Never interpreted.** nvoken does not read it, route on it, or index   it. - **Never model-visible.** It is not rendered into context and no tool   can read it. This is a rule the runtime is held to, not a description   of how it happens to work today. - **Carried in the signed callback envelope**, as a sibling of the   `nvoken` object rather than a member of it.  **What nvoken guarantees is integrity, not authentication.** What creation recorded is what a signed delivery carries, unchanged. nvoken does not verify that the values are true, or that the caller was entitled to assert them. That is yours.  The rule every callback receiver needs, in one sentence: a value repeated in tool input may only agree with the authorization context, never establish it. The model writes tool input; it does not write this.  Same bounds as `metadata`: at most 16 entries, keys of 1–64 bytes from letters, digits, `_`, `.`, `:`, and `-`, values at most 512 bytes of UTF-8. Unlike `metadata`, a `null` value is not accepted — there is no merge here to delete a key from. ")
    on_conflict: Optional[StrictStr] = Field(default='refuse', description="What you are asserting about a Session that already exists. `refuse`, the default, compares every option you sent. `join` says you want to reach whatever Session is there without asserting how it is configured, so `compaction` and `retention` stop conflicting.  `join` never relaxes `authorization_context`, `pinned_revision`, or `user_key`. Those exist to catch a caller acting on the wrong conversation, and a flag that suppressed them would be a way around the check rather than a way to express intent. ")
    __properties: ClassVar[List[str]] = ["compaction", "retention", "pinned_revision", "authorization_context", "on_conflict"]
    ...
    def on_conflict_validate_enum(cls, value):
        ...
        if value not in set(['refuse', 'join']):
            raise ValueError("must be one of enum values ('refuse', 'join')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "on_conflict": obj.get("on_conflict") if obj.get("on_conflict") is not None else 'refuse'
        })
        return _obj
