...
class SessionOptions(BaseModel):
    """
    Settings stored on the Session. Every field is optional, but send at least one.  On a new Session these are saved. When you reach an existing Session, by ID or by key, `compaction` and `retention` are checked rather than applied: matching values are accepted, and a different value returns `session_options_conflict` with `details.conflicting_paths` naming exactly what disagreed. Compaction needs a turn to install it, because the policy is validated against that turn's model.  `metadata` is merged rather than checked, on the same terms as `PATCH /v1/sessions/{session_id}`: a present key replaces, an absent key survives, and it never conflicts. Deleting a key stays a PATCH, since a null value is not accepted here.
    """ # noqa: E501
