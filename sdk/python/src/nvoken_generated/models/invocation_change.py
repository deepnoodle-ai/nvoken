...
class InvocationChange(BaseModel):
    """
    One lifecycle step of one turn, as the Session transcript stream and the JSON transcript deliver it. Order changes by `revision` and fold to the highest one to get current state.  A change carries what a turn's own projection carries about where it stands: `stop_reason` for a turn that ended, `credit_block` for one paused on spend, and `tool_calls` for what its tools are doing, with `arguments` on the ones waiting for you to run them. You never need a second request to find out why a turn is not moving.
    """ # noqa: E501
    ...
    __properties: ClassVar[List[str]] = ["invocation_id", "revision", "status", "stop_reason", "credit_block", "tool_calls", "through_message_sequence", "error", "usage", "provenance", "structured_output", "structured_output_provenance", "occurred_at"]
