...
class InvocationChange(BaseModel):
    """
    One lifecycle step of one turn, as the Session transcript stream and the JSON transcript deliver it. Order changes by `revision` and fold to the highest one to get current state.  A change carries what a turn's own projection carries about where it stands: `stop_reason` for a turn that ended, `pending_tool_calls` for one waiting on you, `credit_block` for one paused on spend, and `tool_calls` for what its tools are doing. You never need a second request to find out why a turn is not moving.
    """ # noqa: E501
    ...
    stop_reason: Optional[InvocationStopReason] = Field(default=None, description="Why the turn stopped. Present once it has stopped, so an `incomplete` change names the limit that ran out. ")
    credit_block: Optional[CreditBlock] = Field(default=None, description="Present while the turn is paused on spending capacity.")
    pending_tool_calls: Optional[List[PendingHostToolCall]] = Field(default=None, description="What a waiting turn is waiting for. Omitted when nothing is pending. ")
    ...
    __properties: ClassVar[List[str]] = ["invocation_id", "revision", "status", "stop_reason", "credit_block", "pending_tool_calls", "tool_calls", "through_message_sequence", "error", "usage", "provenance", "structured_output", "structured_output_provenance", "occurred_at"]
