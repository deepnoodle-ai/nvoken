...
class InvocationChange(BaseModel):
    ...
    session_id: Optional[Annotated[str, Field(min_length=1, strict=True)]] = Field(description="Opaque identifier with the public `sess_` prefix. Treat the body as opaque.")
    content_expires_at: Optional[datetime] = Field(description="Scheduled private-content expiry for a terminal standalone Invocation. Null while it is nonterminal and for conversation-bound work. ")
    ...
    __properties: ClassVar[List[str]] = ["invocation_id", "session_id", "content_expires_at", "revision", "status", "terminal", "stop_reason", "credit_block", "tool_calls", "through_message_sequence", "error", "usage", "provenance", "structured_output", "structured_output_provenance", "occurred_at"]
