...
class ImageInputSource(BaseModel):
    """
    Exactly one source form is required. Inline sources carry media_type and data. URL sources carry url and may declare media_type.
    """ # noqa: E501
    media_type: Optional[StrictStr] = None
    data: Optional[Annotated[str, Field(min_length=1, strict=True, max_length=6990508)]] = Field(default=None, description="Standard padded base64 with no whitespace. Mutually exclusive with url. ")
    url: Optional[Annotated[str, Field(strict=True, max_length=2048)]] = Field(default=None, description="A public HTTPS URL, downloaded once as the turn is created. nvoken stops reading if the response runs past the size limit, and determines the real media type from the bytes it received. `media_type` is optional here; if you do send it, it must match what the bytes actually are. ")
    __properties: ClassVar[List[str]] = ["media_type", "data", "url"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ImageInputSource from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ImageInputSource from a dict"""
        if obj is None:
            return None
