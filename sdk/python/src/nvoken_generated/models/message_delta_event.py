...
class MessageDeltaEvent(BaseModel):
    ...
    content_index: Annotated[int, Field(strict=True, ge=0)] = Field(description="Which content block of the message this fragment extends, counted from 0 in the order the model opened them. It is a preview coordinate only: reasoning blocks are never saved, so the saved message's `content` array can be shorter and this index is not a position in it. ")
    offset: Annotated[int, Field(strict=True, ge=0)] = Field(description="Where this fragment starts within its block: the UTF-8 byte length of every earlier fragment for the same `(message_id, content_index)`. When it exceeds what you have accumulated, a frame was lost; discard the block and wait for the saved message. ")
    ...
    __properties: ClassVar[List[str]] = ["type", "turn_id", "attempt", "message_id", "content_index", "offset", "kind", "delta", "tool_call_id", "name", "emitted_at"]
