...
CONVERSATIONOWNER_ONE_OF_SCHEMAS = ["TenantConversationOwner", "UserConversationOwner"]

class ConversationOwner(BaseModel):
    """
    ConversationOwner
    """
    # data type: TenantConversationOwner
    oneof_schema_1_validator: Optional[TenantConversationOwner] = None
    # data type: UserConversationOwner
    oneof_schema_2_validator: Optional[UserConversationOwner] = None
    actual_instance: Optional[Union[TenantConversationOwner, UserConversationOwner]] = None
    one_of_schemas: Set[str] = { "TenantConversationOwner", "UserConversationOwner" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = ConversationOwner.model_construct()
        error_messages = []
        match = 0
        # validate data type: TenantConversationOwner
        if not isinstance(v, TenantConversationOwner):
            error_messages.append(f"Error! Input type `{type(v)}` is not `TenantConversationOwner`")
        else:
            match += 1
        # validate data type: UserConversationOwner
        if not isinstance(v, UserConversationOwner):
            error_messages.append(f"Error! Input type `{type(v)}` is not `UserConversationOwner`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in ConversationOwner with oneOf schemas: TenantConversationOwner, UserConversationOwner. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in ConversationOwner with oneOf schemas: TenantConversationOwner, UserConversationOwner. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into TenantConversationOwner
        try:
            instance.actual_instance = TenantConversationOwner.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into UserConversationOwner
        try:
            instance.actual_instance = UserConversationOwner.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into ConversationOwner with oneOf schemas: TenantConversationOwner, UserConversationOwner. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ConversationOwner with oneOf schemas: TenantConversationOwner, UserConversationOwner. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], TenantConversationOwner, UserConversationOwner]]:
