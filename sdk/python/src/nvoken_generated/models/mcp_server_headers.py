...
class MCPServerHeaders(BaseModel):
    ...
    MCPServerHeaders
    """ # noqa: E501
    name: Annotated[str, Field(min_length=1, strict=True, max_length=24)] = Field(description="Exact name of one MCP server in the selected Agent Definition.")
    headers: Dict[str, Annotated[str, Field(min_length=1, strict=True)]] = Field(description="Secret request headers, limited to 8 KiB encoded. Routing, framing, proxy, cookie, hop-by-hop, and MCP session headers are rejected. Headers are encrypted per Invocation and never returned. ")
