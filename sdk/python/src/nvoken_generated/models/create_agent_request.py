...
class CreateAgentRequest(BaseModel):
    ...
    definition_id: Optional[Annotated[str, Field(min_length=1, strict=True)]] = Field(default=None, description="Stable App-owned Agent Definition identifier with the public `def_` prefix. Treat the body as opaque.")
    definition_key: Optional[Annotated[str, Field(min_length=1, strict=True, max_length=255)]] = Field(default=None, description="The `definition_key` of the Agent Definition this Agent follows, as an alternative spelling of `definition_id`. Supply exactly one; supplying both or neither is `400`. The key is resolved in the same transaction that resolves the Agent, so a create and a restatement compare the same Definition however it was named. ")
    pinned_revision: Optional[Annotated[int, Field(strict=True, ge=1)]] = None
    __properties: ClassVar[List[str]] = ["tenant_key", "agent_key", "name", "definition_id", "definition_key", "pinned_revision"]
