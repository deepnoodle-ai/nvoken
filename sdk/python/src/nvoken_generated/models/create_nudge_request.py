...
class CreateNudgeRequest(BaseModel):
    ...
    CreateNudgeRequest
    """ # noqa: E501
    content: TurnInput = Field(description="Text guidance for the running turn: a string, or an array of text blocks. Media blocks are refused here. ")
    idempotency_key: Optional[Annotated[str, Field(min_length=1, strict=True, max_length=255)]] = Field(default=None, description="Per-Turn retry key. The same key with the same content returns the original acknowledgement with `deduplicated: true`; the same key with different content is refused, so a key is never reused with different words. ")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "content": TurnInput.from_dict(obj["content"]) if obj.get("content") is not None else None,
