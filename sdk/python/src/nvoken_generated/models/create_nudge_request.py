...
class CreateNudgeRequest(BaseModel):
    ...
    CreateNudgeRequest
    """ # noqa: E501
    content: InvocationInput = Field(description="Text guidance for the running turn: a string, or an array of text blocks. Media blocks are refused here. ")
    idempotency_key: Optional[Annotated[str, Field(min_length=1, strict=True, max_length=255)]] = Field(default=None, description="Per-Invocation retry key. The same key with the same content returns the original acknowledgement with `deduplicated: true`; the same key with different content is refused, so a key is never reused with different words. ")
    __properties: ClassVar[List[str]] = ["content", "idempotency_key"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of CreateNudgeRequest from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of CreateNudgeRequest from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "content": InvocationInput.from_dict(obj["content"]) if obj.get("content") is not None else None,
