...
class ImageInputBlock(BaseModel):
    """
    An image, sent inline or by URL. A URL is downloaded once as the turn is created; nvoken stores those bytes and never fetches the URL again, so the image stays stable even if the link later changes or breaks. The image must be at most 5 MB and 8000 by 8000 pixels, and the model you selected must support images and this exact media type. nvoken determines the real type from the bytes themselves rather than trusting the declared type or the file extension.
    """ # noqa: E501
    type: StrictStr
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ImageInputBlock from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ImageInputBlock from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "source": ImageInputSource.from_dict(obj["source"]) if obj.get("source") is not None else None
        })
        return _obj
