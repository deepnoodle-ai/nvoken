...
class RetentionPolicy(BaseModel):
    """
    How long a Session can sit unused before nvoken deletes it. When the window passes, the Session and everything under it are erased, exactly as `DELETE /v1/sessions/{session_id}` would.  The clock measures idle time, not age: it resets every time a turn starts and every time one finishes. A long-running turn can never expire out from under you.  **Automatic expiry never cancels running work.** A Session holding a queued, running, or waiting Invocation is skipped and reconsidered on the next sweep. An explicit `DELETE` may still stop a running turn, because a person asked for it; a clock may not.  Omitting retention retains the Session until it is deleted explicitly, which stays the default.
    """ # noqa: E501
