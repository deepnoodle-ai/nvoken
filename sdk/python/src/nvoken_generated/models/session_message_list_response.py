...
from nvoken_generated.models.browser_session_message_list import BrowserSessionMessageList
from nvoken_generated.models.session_message_list import SessionMessageList
from pydantic import StrictStr, Field
from typing import Union, List, Set, Optional, Dict
from typing_extensions import Literal, Self

SESSIONMESSAGELISTRESPONSE_ONE_OF_SCHEMAS = ["BrowserSessionMessageList", "SessionMessageList"]

class SessionMessageListResponse(BaseModel):
    """
    SessionMessageListResponse
    """
    # data type: SessionMessageList
    oneof_schema_1_validator: Optional[SessionMessageList] = None
    # data type: BrowserSessionMessageList
    oneof_schema_2_validator: Optional[BrowserSessionMessageList] = None
    actual_instance: Optional[Union[BrowserSessionMessageList, SessionMessageList]] = None
    one_of_schemas: Set[str] = { "BrowserSessionMessageList", "SessionMessageList" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = SessionMessageListResponse.model_construct()
        error_messages = []
        match = 0
        # validate data type: SessionMessageList
        if not isinstance(v, SessionMessageList):
            error_messages.append(f"Error! Input type `{type(v)}` is not `SessionMessageList`")
        else:
            match += 1
        # validate data type: BrowserSessionMessageList
        if not isinstance(v, BrowserSessionMessageList):
            error_messages.append(f"Error! Input type `{type(v)}` is not `BrowserSessionMessageList`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in SessionMessageListResponse with oneOf schemas: BrowserSessionMessageList, SessionMessageList. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in SessionMessageListResponse with oneOf schemas: BrowserSessionMessageList, SessionMessageList. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into SessionMessageList
        try:
            instance.actual_instance = SessionMessageList.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into BrowserSessionMessageList
        try:
            instance.actual_instance = BrowserSessionMessageList.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into SessionMessageListResponse with oneOf schemas: BrowserSessionMessageList, SessionMessageList. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into SessionMessageListResponse with oneOf schemas: BrowserSessionMessageList, SessionMessageList. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], BrowserSessionMessageList, SessionMessageList]]:
