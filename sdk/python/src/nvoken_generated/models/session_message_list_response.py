...
SESSIONMESSAGELISTRESPONSE_ONE_OF_SCHEMAS = ["ClientSessionMessageList", "SessionMessageList"]
...
class SessionMessageListResponse(BaseModel):
    ...
    # data type: ClientSessionMessageList
    oneof_schema_2_validator: Optional[ClientSessionMessageList] = None
    actual_instance: Optional[Union[ClientSessionMessageList, SessionMessageList]] = None
    one_of_schemas: Set[str] = { "ClientSessionMessageList", "SessionMessageList" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if not isinstance(v, SessionMessageList):
            ...
            match += 1
        # validate data type: ClientSessionMessageList
        if not isinstance(v, ClientSessionMessageList):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ClientSessionMessageList`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in SessionMessageListResponse with oneOf schemas: ClientSessionMessageList, SessionMessageList. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in SessionMessageListResponse with oneOf schemas: ClientSessionMessageList, SessionMessageList. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into ClientSessionMessageList
        try:
            instance.actual_instance = ClientSessionMessageList.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into SessionMessageListResponse with oneOf schemas: ClientSessionMessageList, SessionMessageList. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into SessionMessageListResponse with oneOf schemas: ClientSessionMessageList, SessionMessageList. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ClientSessionMessageList, SessionMessageList]]:
