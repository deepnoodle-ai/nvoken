...
class StreamEndEvent(BaseModel):
    """
    This connection is closing. The stream continues, and the turn is unaffected.  There is no cursor here. You already track your last durable one, because you need it to survive a connection that drops without saying anything, and a field that repeats what you must hold anyway is a second spelling of it.
    """ # noqa: E501
    type: StrictStr
    ...
    __properties: ClassVar[List[str]] = ["type", "session_id", "reason"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['stream.end']):
            raise ValueError("must be one of enum values ('stream.end')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of StreamEndEvent from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of StreamEndEvent from a dict"""
        if obj is None:
            return None
