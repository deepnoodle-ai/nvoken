...
DELIVERYBEHAVIORSOURCE_ONE_OF_SCHEMAS = ["DeliveryAgentRevisionSource", "DeliveryInlineSource"]

class DeliveryBehaviorSource(BaseModel):
    """
    DeliveryBehaviorSource
    """
    # data type: DeliveryAgentRevisionSource
    oneof_schema_1_validator: Optional[DeliveryAgentRevisionSource] = None
    # data type: DeliveryInlineSource
    oneof_schema_2_validator: Optional[DeliveryInlineSource] = None
    actual_instance: Optional[Union[DeliveryAgentRevisionSource, DeliveryInlineSource]] = None
    one_of_schemas: Set[str] = { "DeliveryAgentRevisionSource", "DeliveryInlineSource" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = DeliveryBehaviorSource.model_construct()
        error_messages = []
        match = 0
        # validate data type: DeliveryAgentRevisionSource
        if not isinstance(v, DeliveryAgentRevisionSource):
            error_messages.append(f"Error! Input type `{type(v)}` is not `DeliveryAgentRevisionSource`")
        else:
            match += 1
        # validate data type: DeliveryInlineSource
        if not isinstance(v, DeliveryInlineSource):
            error_messages.append(f"Error! Input type `{type(v)}` is not `DeliveryInlineSource`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in DeliveryBehaviorSource with oneOf schemas: DeliveryAgentRevisionSource, DeliveryInlineSource. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in DeliveryBehaviorSource with oneOf schemas: DeliveryAgentRevisionSource, DeliveryInlineSource. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into DeliveryAgentRevisionSource
        try:
            instance.actual_instance = DeliveryAgentRevisionSource.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into DeliveryInlineSource
        try:
            instance.actual_instance = DeliveryInlineSource.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into DeliveryBehaviorSource with oneOf schemas: DeliveryAgentRevisionSource, DeliveryInlineSource. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into DeliveryBehaviorSource with oneOf schemas: DeliveryAgentRevisionSource, DeliveryInlineSource. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], DeliveryAgentRevisionSource, DeliveryInlineSource]]:
