...
class OutputTextDeltaEvent(BaseModel):
    """
    A live preview of assistant text. Not saved, never replayed, and never a message. Accumulate it by `(invocation_id, attempt, iteration, content_index)` and discard it under the rules in Streaming above.
    """ # noqa: E501
    type: StrictStr
    ...
    __properties: ClassVar[List[str]] = ["type", "session_id", "invocation_id", "attempt", "iteration", "content_index", "message_id", "text", "emitted_at"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['output_text.delta']):
            raise ValueError("must be one of enum values ('output_text.delta')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of OutputTextDeltaEvent from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of OutputTextDeltaEvent from a dict"""
        if obj is None:
            return None
