...
class ConversationMessage(BaseModel):
    ...
    phase: Optional[MessagePhase] = Field(default=None, description="Whether this assistant message is the turn's actual answer or something along the way. Absent on user and tool messages, which have no phase.  It is worked out when you read, from how the turn ended, and it is frozen onto forked copies so a fork keeps the phases it had. Your own input never carries a phase.  Because it is worked out on read, it is authoritative on ordinary reads and not on the stream: a message delivered there before its turn settled carries `commentary` forever, and no later frame corrects it. On the stream, read `final_answer_message_id` on the turn's current change instead. It names the turn's last assistant message once the turn settled `completed` with stop reason `end_turn`, and is absent otherwise: the same rule nvoken applies when it works out `phase` on read. ")
    created_at: datetime
