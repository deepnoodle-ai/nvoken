...
class ModelToolCapabilities(BaseModel):
    """
    ModelToolCapabilities
    """ # noqa: E501
    choice: ModelToolChoiceCapabilities
    web_search: StrictBool = Field(description="Whether this model can run the provider's own web search. If it reports false, asking for `provider_tools` is refused rather than silently ignored — and a model missing from nvoken's catalog reports nothing, so it is refused too. ")
    __properties: ClassVar[List[str]] = ["choice", "web_search"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ModelToolCapabilities from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ModelToolCapabilities from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "choice": ModelToolChoiceCapabilities.from_dict(obj["choice"]) if obj.get("choice") is not None else None,
