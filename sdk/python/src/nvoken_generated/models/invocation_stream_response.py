...
INVOCATIONSTREAMRESPONSE_ONE_OF_SCHEMAS = ["ClientInvocationStreamEvent", "InvocationStreamEvent"]
...
class InvocationStreamResponse(BaseModel):
    ...
    # data type: ClientInvocationStreamEvent
    oneof_schema_2_validator: Optional[ClientInvocationStreamEvent] = None
    actual_instance: Optional[Union[ClientInvocationStreamEvent, InvocationStreamEvent]] = None
    one_of_schemas: Set[str] = { "ClientInvocationStreamEvent", "InvocationStreamEvent" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if not isinstance(v, InvocationStreamEvent):
            ...
            match += 1
        # validate data type: ClientInvocationStreamEvent
        if not isinstance(v, ClientInvocationStreamEvent):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ClientInvocationStreamEvent`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in InvocationStreamResponse with oneOf schemas: ClientInvocationStreamEvent, InvocationStreamEvent. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in InvocationStreamResponse with oneOf schemas: ClientInvocationStreamEvent, InvocationStreamEvent. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into ClientInvocationStreamEvent
        try:
            instance.actual_instance = ClientInvocationStreamEvent.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into InvocationStreamResponse with oneOf schemas: ClientInvocationStreamEvent, InvocationStreamEvent. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into InvocationStreamResponse with oneOf schemas: ClientInvocationStreamEvent, InvocationStreamEvent. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ClientInvocationStreamEvent, InvocationStreamEvent]]:
