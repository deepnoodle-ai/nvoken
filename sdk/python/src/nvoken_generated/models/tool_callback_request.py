...
class ToolCallbackRequest(BaseModel):
    ...
    authorization_context: Optional[Dict[str, Annotated[str, Field(strict=True, max_length=512)]]] = Field(default=None, description="The conversation or standalone turn's `authorization_context`, absent when admission did not set one.  It sits beside `nvoken` rather than inside it on purpose. Everything in `nvoken` is a fact nvoken minted or resolved; this is a value you asserted at creation and nvoken carried unchanged. Signing proves it reached you as recorded, not that it is true.  A value repeated in tool input may only agree with this, never establish it. ")
    input: Optional[Any]
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["nvoken", "authorization_context", "input"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ToolCallbackRequest from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ToolCallbackRequest from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "nvoken": ToolCallbackContext.from_dict(obj["nvoken"]) if obj.get("nvoken") is not None else None,
