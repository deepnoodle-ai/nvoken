...
class ToolCallbackRequest(BaseModel):
    ...
    authorization_context: Optional[Dict[str, Annotated[str, Field(strict=True, max_length=512)]]] = Field(default=None, description="The Session's `authorization_context`, absent when the Session was created without one.  It sits beside `nvoken` rather than inside it on purpose. Everything in `nvoken` is a fact nvoken minted or resolved; this is a value you asserted at creation and nvoken carried unchanged. Signing proves it reached you as recorded, not that it is true.  A value repeated in tool input may only agree with this, never establish it. ")
    input: Optional[Any]
