...
TURNBEHAVIORSELECTION_ONE_OF_SCHEMAS = ["AgentTurnBehavior", "InlineTurnBehavior"]

class TurnBehaviorSelection(BaseModel):
    """
    TurnBehaviorSelection
    """
    # data type: AgentTurnBehavior
    oneof_schema_1_validator: Optional[AgentTurnBehavior] = None
    # data type: InlineTurnBehavior
    oneof_schema_2_validator: Optional[InlineTurnBehavior] = None
    actual_instance: Optional[Union[AgentTurnBehavior, InlineTurnBehavior]] = None
    one_of_schemas: Set[str] = { "AgentTurnBehavior", "InlineTurnBehavior" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = TurnBehaviorSelection.model_construct()
        error_messages = []
        match = 0
        # validate data type: AgentTurnBehavior
        if not isinstance(v, AgentTurnBehavior):
            error_messages.append(f"Error! Input type `{type(v)}` is not `AgentTurnBehavior`")
        else:
            match += 1
        # validate data type: InlineTurnBehavior
        if not isinstance(v, InlineTurnBehavior):
            error_messages.append(f"Error! Input type `{type(v)}` is not `InlineTurnBehavior`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in TurnBehaviorSelection with oneOf schemas: AgentTurnBehavior, InlineTurnBehavior. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in TurnBehaviorSelection with oneOf schemas: AgentTurnBehavior, InlineTurnBehavior. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into AgentTurnBehavior
        try:
            instance.actual_instance = AgentTurnBehavior.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into InlineTurnBehavior
        try:
            instance.actual_instance = InlineTurnBehavior.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into TurnBehaviorSelection with oneOf schemas: AgentTurnBehavior, InlineTurnBehavior. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into TurnBehaviorSelection with oneOf schemas: AgentTurnBehavior, InlineTurnBehavior. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], AgentTurnBehavior, InlineTurnBehavior]]:
