...
pub struct Sampling {
    /// Sampling temperature, expressed the same way across providers. Omit `sampling` entirely to keep the provider's own default; an explicit `0` means 0 and is not treated as absent.  If the model is not known to support temperature, the turn is refused rather than run with the setting quietly dropped.
    #[serde(rename = "temperature")]
    pub temperature: f64,
}

impl Sampling {
    pub fn new(temperature: f64) -> Sampling {
