...
/// ToolCallStatus : Where one tool call stands. `pending` means the model asked for it and nothing has started it yet; `running` means it is executing or, for a host tool, waiting on you. The last three are final.
/// Where one tool call stands. `pending` means the model asked for it and nothing has started it yet; `running` means it is executing or, for a host tool, waiting on you. The last three are final.
