...
/// CompactionPolicyTriggerTokens : Estimated retained-context size that triggers compaction. The Session stores \"auto\" as an exact value materialized when the policy is installed to 60% of that Invocation's primary model context window, clamped to the supported integer range.
...
#[serde(untagged)]
pub enum CompactionPolicyTriggerTokens {
    Integer(u32),
    String(String),
}

impl Default for CompactionPolicyTriggerTokens {
    fn default() -> Self {
