...
/// CompactionPolicyTriggerTokens : Estimated retained-context size that triggers compaction. The Conversation stores \"auto\" as an exact value materialized when the policy is installed to 60% of that Turn's primary model context window, clamped to the supported integer range.
