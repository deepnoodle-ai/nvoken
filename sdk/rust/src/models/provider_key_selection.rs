...
/// ProviderKeySelection : Exactly one selection matching the canonical model provider. Omitting `provider_keys` entirely resolves the default chain instead: the app's stored key when one exists, else the self-hosted installation's environment key (config_byok), else platform funding when the installation enables it.
...
#[serde(untagged)]
pub enum ProviderKeySelection {
    ProviderKeySelectionOneOf(Box<models::ProviderKeySelectionOneOf>),
    ProviderKeySelectionOneOf1(Box<models::ProviderKeySelectionOneOf1>),
}

impl Default for ProviderKeySelection {
    fn default() -> Self {
        Self::ProviderKeySelectionOneOf(Default::default())
    }
}
///
