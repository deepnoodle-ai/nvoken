...
/// SessionBudget : A spending cap in USD covering every turn in this Session. You can change it at any time.  The figure is nvoken's own estimate, based on published list prices and the model calls it recorded — it is a control for stopping runaway work, not a bill. If a model has no price in nvoken's catalog, turns using it are refused rather than run uncosted.
...
impl SessionBudget {
    ...
    pub fn new(max_estimated_cost_usd: f64) -> SessionBudget {
        SessionBudget {
