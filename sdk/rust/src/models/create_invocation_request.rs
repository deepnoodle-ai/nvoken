...
pub struct CreateInvocationRequest {
    ...
    /// Conversation intent. Omit this object for a standalone Invocation; nvoken still uses an expiring internal carrier, but it is not a public Session and every response reports `session_id: null`.
    #[serde(rename = "session", skip_serializing_if = "Option::is_none")]
    pub session: Option<Box<models::InvocationSession>>,
    /// Retention for a newly created conversation or a standalone turn. Standalone turns default to one hour. Invalid when continuing an existing Session.
    ...
    pub authorization_context: Option<std::collections::HashMap<String, String>>,
