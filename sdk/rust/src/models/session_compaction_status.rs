...
/// SessionCompactionStatus : `applied` means the new summary took effect and is what the model now sees in place of the older messages. `fell_through` means the summary attempt failed or came back unusable, so the turn carried on with the previous summary. Either way the turn keeps running — a failed summary is not a failed turn.
/// `applied` means the new summary took effect and is what the model now sees in place of the older messages. `fell_through` means the summary attempt failed or came back unusable, so the turn carried on with the previous summary. Either way the turn keeps running — a failed summary is not a failed turn.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum SessionCompactionStatus {
    #[serde(rename = "applied")]
    Applied,
    #[serde(rename = "fell_through")]
    FellThrough,
}

impl std::fmt::Display for SessionCompactionStatus {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            Self::Applied => write!(f, "applied"),
            Self::FellThrough => write!(f, "fell_through"),
        }
    }
}

impl Default for SessionCompactionStatus {
    fn default() -> SessionCompactionStatus {
        Self::Applied
    }
}
