...
/// InputBlock : One caller-input block. A block carries text, an image, or a document; the members of one type are never valid on another.
...
#[serde(untagged)]
pub enum InputBlock {
    TextInputBlock(Box<models::TextInputBlock>),
    ImageInputBlock(Box<models::ImageInputBlock>),
    DocumentInputBlock(Box<models::DocumentInputBlock>),
}

impl Default for InputBlock {
    fn default() -> Self {
        Self::TextInputBlock(Default::default())
    }
}
///
