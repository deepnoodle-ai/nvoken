...
/// AgentDefinition : Immutable Agent Definition returned by registration and detailed Invocation reads. The host owns the configuration and nvoken only records the Agent Definition each turn ran with. Invocation creation nests these execution fields under `agent_definition`; nvoken stores one immutable Agent Definition and re-reads it unchanged on every model iteration and recovery. Two registrations and Invocations with byte-identical execution fields share one Agent Definition. Identity, session selection, input, idempotency, `if_active`, `webhook`, metadata, and provider key selection are durable elsewhere and never appear here. Invocation admission for callback declarations requires App callback-signing configuration. Remote MCP authentication headers are encrypted outside this Agent Definition and never returned. Tools or structured output require at least two model iterations; omission resolves to three or the lower installation maximum.
...
impl AgentDefinition {
    /// Immutable Agent Definition returned by registration and detailed Invocation reads. The host owns the configuration and nvoken only records the Agent Definition each turn ran with. Invocation creation nests these execution fields under `agent_definition`; nvoken stores one immutable Agent Definition and re-reads it unchanged on every model iteration and recovery. Two registrations and Invocations with byte-identical execution fields share one Agent Definition. Identity, session selection, input, idempotency, `if_active`, `webhook`, metadata, and provider key selection are durable elsewhere and never appear here. Invocation admission for callback declarations requires App callback-signing configuration. Remote MCP authentication headers are encrypted outside this Agent Definition and never returned. Tools or structured output require at least two model iterations; omission resolves to three or the lower installation maximum.
