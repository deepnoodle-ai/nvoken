...
/// CallbackDeliveryOutcome : `pending` means nvoken has not reached a final transport outcome. `acknowledged` means the endpoint returned 202 with an empty body and the ToolCall still awaits settlement. The other values are terminal transport outcomes.
/// `pending` means nvoken has not reached a final transport outcome. `acknowledged` means the endpoint returned 202 with an empty body and the ToolCall still awaits settlement. The other values are terminal transport outcomes.
...
impl std::fmt::Display for CallbackDeliveryOutcome {
...
impl Default for CallbackDeliveryOutcome {
    fn default() -> CallbackDeliveryOutcome {
        Self::Pending
    }
}
