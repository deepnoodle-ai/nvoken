...
/// RetentionPolicy : Bounds how long an idle Session is retained. Past the window nvoken erases the Session and its whole subtree, exactly as `DELETE /v1/sessions/{session_id}` would.  The window measures idle time, not lifetime: it restarts on every Invocation admission and every settlement, so a turn that runs longer than the window cannot expire underneath itself.  **Automatic expiry never cancels running work.** A Session holding a queued, running, or waiting Invocation is skipped and reconsidered on the next sweep. An explicit `DELETE` may still stop a running turn, because a person asked for it; a clock may not.  Omitting retention retains the Session until it is deleted explicitly, which stays the default.
...
impl RetentionPolicy {
    /// Bounds how long an idle Session is retained. Past the window nvoken erases the Session and its whole subtree, exactly as `DELETE /v1/sessions/{session_id}` would.  The window measures idle time, not lifetime: it restarts on every Invocation admission and every settlement, so a turn that runs longer than the window cannot expire underneath itself.  **Automatic expiry never cancels running work.** A Session holding a queued, running, or waiting Invocation is skipped and reconsidered on the next sweep. An explicit `DELETE` may still stop a running turn, because a person asked for it; a clock may not.  Omitting retention retains the Session until it is deleted explicitly, which stays the default.
    pub fn new(ttl_seconds: u32) -> RetentionPolicy {
        RetentionPolicy { ttl_seconds }
    }
}
