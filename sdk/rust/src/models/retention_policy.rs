...
pub struct RetentionPolicy {
    ...
    #[serde(rename = "max_expires_at", skip_serializing_if = "Option::is_none")]
    pub max_expires_at: Option<chrono::DateTime<chrono::FixedOffset>>,
}

impl RetentionPolicy {
    /// How long a Conversation can sit unused before nvoken deletes it. When the window passes, the Conversation and everything under it are erased, exactly as `DELETE /v1/conversations/{conversation_id}` would.  The clock measures idle time, not age: it resets every time a turn starts and every time one finishes. A long-running turn can never expire out from under you.  Expiry and explicit erasure never cancel active work. A Conversation holding a nonterminal Turn is skipped by expiry and refused by explicit erasure until that Turn becomes terminal.  Omitting retention retains the Conversation until it is deleted explicitly, which stays the default.
    pub fn new(ttl_seconds: u32) -> RetentionPolicy {
        RetentionPolicy {
            ttl_seconds,
            max_expires_at: None,
        }
