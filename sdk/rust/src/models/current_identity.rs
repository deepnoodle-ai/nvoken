...
/// CurrentIdentity : Who nvoken thinks you are. `method` says how you authenticated. A machine API key carries `credential_id` and `type`; a short-lived console presentation may carry `org_id`; a browser grant carries its fixed App, Agent, AgentRevision, tenant, subject, memory grant, and Conversation grant.
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct CurrentIdentity {
    #[serde(rename = "authentication")]
    pub authentication: Box<models::CurrentIdentityAuthentication>,
}

impl CurrentIdentity {
    /// Who nvoken thinks you are. `method` says how you authenticated. A machine API key carries `credential_id` and `type`; a short-lived console presentation may carry `org_id`; a browser grant carries its fixed App, Agent, AgentRevision, tenant, subject, memory grant, and Conversation grant.
