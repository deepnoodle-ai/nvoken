...
/// StreamResyncReason : Why previews were invalidated. `live_delivery_gap` means the live delivery path could not prove continuity, so previews were lost; discard what you accumulated and wait for saved frames.  Expect new values here over time. Treat a value you do not recognize the same way you treat `live_delivery_gap`.
/// Why previews were invalidated. `live_delivery_gap` means the live delivery path could not prove continuity, so previews were lost; discard what you accumulated and wait for saved frames.  Expect new values here over time. Treat a value you do not recognize the same way you treat `live_delivery_gap`.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum StreamResyncReason {
    #[serde(rename = "live_delivery_gap")]
    ReasonLiveDeliveryGap,
}

impl std::fmt::Display for StreamResyncReason {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            Self::ReasonLiveDeliveryGap => write!(f, "live_delivery_gap"),
        }
    }
}

impl Default for StreamResyncReason {
    fn default() -> StreamResyncReason {
