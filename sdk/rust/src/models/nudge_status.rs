...
/// NudgeStatus : `pending` is the only state a turn will still pick up. `drained` means the turn took it and it is now in the transcript. `expired` means the turn ended without ever taking it. `cancelled` means you withdrew it before the turn got to it.
...
impl std::fmt::Display for NudgeStatus {
...
impl Default for NudgeStatus {
    fn default() -> NudgeStatus {
        Self::Pending
    }
}
