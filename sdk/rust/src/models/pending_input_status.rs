...
/// PendingInputStatus : `pending` is the only state the turn consumes. `drained` means the executor promoted the content into the transcript, `expired` that the Invocation settled without taking it, and `cancelled` that the caller withdrew it first.
/// `pending` is the only state the turn consumes. `drained` means the executor promoted the content into the transcript, `expired` that the Invocation settled without taking it, and `cancelled` that the caller withdrew it first.
