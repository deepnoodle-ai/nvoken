...
pub struct CreateNudgeRequest {
    ...
    /// Per-Invocation retry key. The same key with the same content returns the original acknowledgement with `deduplicated: true`; the same key with different content is refused, so a key is never reused with different words.
    #[serde(rename = "idempotency_key", skip_serializing_if = "Option::is_none")]
    pub idempotency_key: Option<String>,
}

impl CreateNudgeRequest {
    pub fn new(content: models::InvocationInput) -> CreateNudgeRequest {
        CreateNudgeRequest {
