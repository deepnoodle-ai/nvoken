...
/// DocumentInputSource : Exactly one source form is required. Inline sources carry media_type and data. URL sources carry url and may declare media_type.
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct DocumentInputSource {
    #[serde(rename = "media_type", skip_serializing_if = "Option::is_none")]
    pub media_type: Option<MediaType>,
    /// Standard padded base64 with no whitespace.
    #[serde(rename = "data", skip_serializing_if = "Option::is_none")]
    pub data: Option<String>,
    /// A public HTTPS URL, downloaded once as the turn is created. nvoken stops reading if the response runs past the size limit, and determines the real media type from the bytes it received. `media_type` is optional here; if you do send it, it must be `application/pdf`.
    #[serde(rename = "url", skip_serializing_if = "Option::is_none")]
    pub url: Option<String>,
}

impl DocumentInputSource {
    ...
    pub fn new() -> DocumentInputSource {
        DocumentInputSource {
            media_type: None,
            data: None,
            url: None,
