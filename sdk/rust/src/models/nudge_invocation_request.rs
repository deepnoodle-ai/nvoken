...
pub struct NudgeInvocationRequest {
    /// Text guidance for the running turn: a string, or an array of text blocks. Media blocks are refused here.
    #[serde(rename = "content")]
    pub content: Box<models::InvocationInput>,
    /// Per-Invocation retry key. The same key with the same content returns the original acknowledgement with `deduped: true`; the same key with different content is refused, so a key is never reused with different words.
    #[serde(rename = "idempotency_key", skip_serializing_if = "Option::is_none")]
    pub idempotency_key: Option<String>,
}

impl NudgeInvocationRequest {
    pub fn new(content: models::InvocationInput) -> NudgeInvocationRequest {
