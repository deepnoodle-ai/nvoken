...
pub struct StreamResyncEvent {
    #[serde(rename = "type")]
    pub r#type: Type,
    /// The Turn whose previews are void. On a Turn stream it is always present and names that Turn. On a Conversation stream it is present when one Turn's previews are void and absent when every preview in the Conversation is: an absent field is scope, where a null identifier would be scope wearing an identifier's name.
...
impl StreamResyncEvent {
    pub fn new(r#type: Type, reason: models::StreamResyncReason) -> StreamResyncEvent {
        StreamResyncEvent {
            r#type,
            turn_id: None,
            reason,
