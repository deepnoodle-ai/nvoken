...
/// ImageInputSource : Exactly one source form is required. Inline sources carry media_type and data. URL sources carry url and may declare media_type.
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct ImageInputSource {
    #[serde(rename = "media_type", skip_serializing_if = "Option::is_none")]
    pub media_type: Option<MediaType>,
    /// Standard padded base64 with no whitespace. Mutually exclusive with url.
    #[serde(rename = "data", skip_serializing_if = "Option::is_none")]
    pub data: Option<String>,
    /// A public HTTPS URL, downloaded once as the turn is created. nvoken stops reading if the response runs past the size limit, and determines the real media type from the bytes it received. `media_type` is optional here; if you do send it, it must match what the bytes actually are.
