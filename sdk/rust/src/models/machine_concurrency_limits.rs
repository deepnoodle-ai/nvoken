...
/// MachineConcurrencyLimits : Optional machine-credential fairness limits enforced transactionally at Turn admission. The counts include nonterminal Turns admitted through any credential class in the same tenant or user.
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct MachineConcurrencyLimits {
    /// Maximum nonterminal Turns in the resolved tenant.
    #[serde(rename = "max_concurrent_turns_per_tenant")]
    pub max_concurrent_turns_per_tenant: u64,
    /// Maximum nonterminal Turns for one user_key in the resolved tenant. Machine admissions without user_key are governed only by the tenant and App-wide ceilings.
    #[serde(rename = "max_concurrent_turns_per_user")]
    pub max_concurrent_turns_per_user: u64,
}

impl MachineConcurrencyLimits {
    /// Optional machine-credential fairness limits enforced transactionally at Turn admission. The counts include nonterminal Turns admitted through any credential class in the same tenant or user.
    pub fn new(
        max_concurrent_turns_per_tenant: u64,
        max_concurrent_turns_per_user: u64,
    ) -> MachineConcurrencyLimits {
        MachineConcurrencyLimits {
            max_concurrent_turns_per_tenant,
