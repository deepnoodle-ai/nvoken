...
/// DocumentInputBlock : An inline or URL document. A URL is fetched once during admission. nvoken stores the fetched bytes and never fetches it again. Admission applies the same sniff rules as an image with a 16777216-byte bound. Page limits are the provider's, so an oversized page count settles as input_media_rejected.
...
impl DocumentInputBlock {
    /// An inline or URL document. A URL is fetched once during admission. nvoken stores the fetched bytes and never fetches it again. Admission applies the same sniff rules as an image with a 16777216-byte bound. Page limits are the provider's, so an oversized page count settles as input_media_rejected.
