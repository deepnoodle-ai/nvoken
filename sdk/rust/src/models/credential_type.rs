...
/// CredentialType : Durable trust boundary. App and App-read-only keys require `app_id`; installation-admin keys forbid an App or Org binding.
/// Durable trust boundary. App and App-read-only keys require `app_id`; installation-admin keys forbid an App or Org binding.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum CredentialType {
    #[serde(rename = "installation_admin")]
    InstallationAdmin,
    #[serde(rename = "app")]
    App,
...
impl std::fmt::Display for CredentialType {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            Self::InstallationAdmin => write!(f, "installation_admin"),
            Self::App => write!(f, "app"),
            Self::AppReadOnly => write!(f, "app_read_only"),
        }
    }
}
