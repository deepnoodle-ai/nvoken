...
/// ToolCallResultOrigin : The component that committed the result. `callback` is an inline callback response; `host` includes settlement through `tool-results`.
/// The component that committed the result. `callback` is an inline callback response; `host` includes settlement through `tool-results`.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum ToolCallResultOrigin {
...
impl std::fmt::Display for ToolCallResultOrigin {
...
impl Default for ToolCallResultOrigin {
    fn default() -> ToolCallResultOrigin {
        Self::Builtin
    }
}
