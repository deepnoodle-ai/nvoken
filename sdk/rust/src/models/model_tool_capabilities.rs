...
pub struct ModelToolCapabilities {
    ...
    /// Whether this exact model supports the provider's server-side web search. Admission fails closed on it, so `provider_tools` is refused on a model that reports false — including any uncataloged model, which reports nothing.
