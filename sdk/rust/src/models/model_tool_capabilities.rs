...
pub struct ModelToolCapabilities {
    #[serde(rename = "choice")]
    pub choice: Box<models::ModelToolChoiceCapabilities>,
    /// Whether this model can run the provider's own web search. If it reports false, asking for `provider_tools` is refused rather than silently ignored — and a model missing from nvoken's catalog reports nothing, so it is refused too.
...
impl ModelToolCapabilities {
    pub fn new(
        choice: models::ModelToolChoiceCapabilities,
        web_search: bool,
