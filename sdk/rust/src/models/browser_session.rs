...
pub struct BrowserSession {
    ...
    /// Tool calls of the active Invocation, with their current status. `arguments` and `deadline_at` appear only on eligible named host-mode calls declared by the admitted definition, which are the only ones a browser may answer.
