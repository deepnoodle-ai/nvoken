...
pub struct NudgeAcknowledgement {
    /// Opaque identifier with the public `input_` prefix. Treat the body as opaque.
    #[serde(rename = "pending_input_id")]
    pub pending_input_id: String,
    #[serde(rename = "state")]
    pub state: models::PendingInputStatus,
    #[serde(rename = "deduped")]
    pub deduped: bool,
    /// Transcript position to watch from for the promoted message. It is the Session cursor as of this call for input that has not been drained, and the position immediately before the promoted message for input that already has, so reading after it finds the message either way.
    #[serde(rename = "after_sequence")]
    pub after_sequence: i64,
}

impl NudgeAcknowledgement {
    pub fn new(
        pending_input_id: String,
        state: models::PendingInputStatus,
        deduped: bool,
        after_sequence: i64,
