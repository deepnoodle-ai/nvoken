...
pub struct NudgeAcknowledgement {
    ...
    /// Transcript position to watch from for the promoted message. It is the Session cursor as of this call for a Nudge that has not been drained, and the position immediately before the promoted message for a Nudge that already has, so reading after it finds the message either way.
