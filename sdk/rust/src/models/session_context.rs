...
pub struct SessionContext {
    /// Roughly how many tokens the model will see on the next turn: the most recent summary, plus the messages that came after it. This is the same number nvoken checks when deciding whether to summarize again.
    #[serde(rename = "estimated_tokens")]
    pub estimated_tokens: u64,
    /// Cataloged context window for `model`, or null when the selected model is not in this nvoken installation's catalog.
    #[serde(
        rename = "context_window_tokens",
        deserialize_with = "Option::deserialize"
    )]
    pub context_window_tokens: Option<u32>,
    /// Model whose context window applies: the Session compaction model when configured, otherwise the most recent Invocation's primary model.
    #[serde(rename = "model")]
