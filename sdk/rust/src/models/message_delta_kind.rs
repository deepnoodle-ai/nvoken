...
/// MessageDeltaKind : What this fragment is part of. `text` is assistant prose, `thinking` is reasoning, and `tool_arguments` is the JSON the model is writing for a tool call, which is only valid JSON once complete.  Expect new values here over time. Treat a value you do not recognize as content you do not render, and keep reading.
/// What this fragment is part of. `text` is assistant prose, `thinking` is reasoning, and `tool_arguments` is the JSON the model is writing for a tool call, which is only valid JSON once complete.  Expect new values here over time. Treat a value you do not recognize as content you do not render, and keep reading.
...
pub enum MessageDeltaKind {
    ...
    DeltaKindThinking,
    #[serde(rename = "tool_arguments")]
    DeltaKindToolArguments,
}

impl std::fmt::Display for MessageDeltaKind {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            Self::DeltaKindText => write!(f, "text"),
            Self::DeltaKindThinking => write!(f, "thinking"),
            Self::DeltaKindToolArguments => write!(f, "tool_arguments"),
        }
    }
}

impl Default for MessageDeltaKind {
    fn default() -> MessageDeltaKind {
        Self::DeltaKindText
    }
}
