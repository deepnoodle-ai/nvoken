...
/// TurnWebhookSubject : A pointer to the turn, not a projection of it. It deliberately carries no transcript content, tool arguments, structured output, usage, provenance, or failure message: a signed body that grew those would be a second, staler copy of the record, and a receiver that read them would be reconciling against the wrong source. Read `GET /v1/turns/{turn_id}` or its `/result` for anything beyond what is here.
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct TurnWebhookSubject {
    #[serde(rename = "status")]
    ...
    /// The host tools this turn is parked on. Present on `turn.waiting` only.
    #[serde(
...
impl TurnWebhookSubject {
    /// A pointer to the turn, not a projection of it. It deliberately carries no transcript content, tool arguments, structured output, usage, provenance, or failure message: a signed body that grew those would be a second, staler copy of the record, and a receiver that read them would be reconciling against the wrong source. Read `GET /v1/turns/{turn_id}` or its `/result` for anything beyond what is here.
    pub fn new(status: models::TurnStatus) -> TurnWebhookSubject {
        TurnWebhookSubject {
            status,
            stop_reason: None,
            failure_code: None,
