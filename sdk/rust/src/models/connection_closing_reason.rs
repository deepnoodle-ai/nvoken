...
/// ConnectionClosingReason : Why this connection is closing. `settled` means this Turn stream has delivered the turn's terminal change and is done; do not reconnect, and read the Turn if you want its composed result. `rotate` means the server is cycling the connection, so reconnect now with your last `cursor`. `idle` means no turn is running and the server is reclaiming the connection, so reconnect when you next need to read; nothing is lost while you are away. `slow_consumer` means this connection could not keep up with what was being written to it and was dropped; reconnect, and read faster or buffer more.  Expect new values here over time. Treat a value you do not recognize as `rotate` and reconnect with your last `cursor`. Reconnecting is always safe: a settled turn answers with its rows and `settled` again.
/// Why this connection is closing. `settled` means this Turn stream has delivered the turn's terminal change and is done; do not reconnect, and read the Turn if you want its composed result. `rotate` means the server is cycling the connection, so reconnect now with your last `cursor`. `idle` means no turn is running and the server is reclaiming the connection, so reconnect when you next need to read; nothing is lost while you are away. `slow_consumer` means this connection could not keep up with what was being written to it and was dropped; reconnect, and read faster or buffer more.  Expect new values here over time. Treat a value you do not recognize as `rotate` and reconnect with your last `cursor`. Reconnecting is always safe: a settled turn answers with its rows and `settled` again.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum ConnectionClosingReason {
...
impl std::fmt::Display for ConnectionClosingReason {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            Self::ReasonSettled => write!(f, "settled"),
            Self::ReasonRotate => write!(f, "rotate"),
            Self::ReasonIdle => write!(f, "idle"),
            Self::ReasonSlowConsumer => write!(f, "slow_consumer"),
        }
    }
}

impl Default for ConnectionClosingReason {
    fn default() -> ConnectionClosingReason {
        Self::ReasonSettled
    }
}
