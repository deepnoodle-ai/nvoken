...
/// ConnectionClosingReason : Why this connection is closing. `rotate` means the server is cycling the connection, so reconnect now with your last `cursor`. `idle` means no turn is running and the server is reclaiming the connection, so reconnect when you next need to read; nothing is lost while you are away. `slow_consumer` means this connection could not keep up with what was being written to it and was dropped; reconnect, and read faster or buffer more.  Expect new values here over time. Treat a value you do not recognize as `rotate` and reconnect with your last `cursor`. Reconnecting is always safe.
/// Why this connection is closing. `rotate` means the server is cycling the connection, so reconnect now with your last `cursor`. `idle` means no turn is running and the server is reclaiming the connection, so reconnect when you next need to read; nothing is lost while you are away. `slow_consumer` means this connection could not keep up with what was being written to it and was dropped; reconnect, and read faster or buffer more.  Expect new values here over time. Treat a value you do not recognize as `rotate` and reconnect with your last `cursor`. Reconnecting is always safe.
