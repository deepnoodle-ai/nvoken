...
/// WebhookEvent : `invocation.waiting` fires when the Invocation parks with at least one unresolved host ToolCall the caller must satisfy; a wait made entirely of callback tools emits nothing, because nvoken delivers those itself. `invocation.paused` fires when an opt-in consumption budget parks the turn and carries the exhausted `stop_reason`. `invocation.settled` fires once when the Invocation reaches `completed`, `incomplete`, `failed`, or `cancelled`; completed and incomplete payloads carry `stop_reason` next to `failure_code`.
/// `invocation.waiting` fires when the Invocation parks with at least one unresolved host ToolCall the caller must satisfy; a wait made entirely of callback tools emits nothing, because nvoken delivers those itself. `invocation.paused` fires when an opt-in consumption budget parks the turn and carries the exhausted `stop_reason`. `invocation.settled` fires once when the Invocation reaches `completed`, `incomplete`, `failed`, or `cancelled`; completed and incomplete payloads carry `stop_reason` next to `failure_code`.
...
pub enum WebhookEvent {
    ...
    WebhookEventPaused,
    #[serde(rename = "invocation.settled")]
    WebhookEventSettled,
}
...
impl std::fmt::Display for WebhookEvent {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            ...
            Self::WebhookEventSettled => write!(f, "invocation.settled"),
        }
    }
}
