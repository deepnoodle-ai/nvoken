...
pub struct SessionCompaction {
    ...
    /// What the summarizing model call cost, when it was recorded reliably. This counts toward the Session's usage but never toward the usage of the turn that triggered it.
    #[serde(rename = "usage", deserialize_with = "Option::deserialize")]
    pub usage: Option<Box<models::ModelUsage>>,
    /// The summary text itself, for an attempt that took effect; null for one that did not. It is shown here so you can see what the model is working from. It is not a message in the conversation and never appears in the transcript.
