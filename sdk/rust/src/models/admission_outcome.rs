...
/// AdmissionOutcome : `admitted` created a Turn; `refused` did not. A refusal is the only record that the request happened at all.
/// `admitted` created a Turn; `refused` did not. A refusal is the only record that the request happened at all.
