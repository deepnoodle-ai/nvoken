...
/// AdmissionOutcome : `admitted` created an Invocation; `refused` did not. A refusal is the only record that the request happened at all.
/// `admitted` created an Invocation; `refused` did not. A refusal is the only record that the request happened at all.
...
impl Default for AdmissionOutcome {
    fn default() -> AdmissionOutcome {
        Self::Admitted
    }
}
