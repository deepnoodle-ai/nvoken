...
/// ResumeTurnRequest : Replacement for the one turn-level consumption ceiling named by the held Turn's `stop_reason`. Exactly that field must be present and must be raised above both the previous ceiling and usage so far.
...
impl ResumeTurnRequest {
    /// Replacement for the one turn-level consumption ceiling named by the held Turn's `stop_reason`. Exactly that field must be present and must be raised above both the previous ceiling and usage so far.
    pub fn new(limits: models::Limits) -> ResumeTurnRequest {
        ResumeTurnRequest {
