...
/// ThinkingDeltaEvent : A live preview of the model's reasoning, carried on machine and browser streams alike. What an end user sees is your application's decision, made in your application.  Reasoning is watchable and not stored: no content block carries it, and no read returns it. Watch the stream if you want to see the model think. A turn that runs under explicit `reasoning` controls emits no reasoning previews at all, to either audience.
