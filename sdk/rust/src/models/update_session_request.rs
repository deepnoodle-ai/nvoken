...
pub struct UpdateSessionRequest {
    /// Metadata merge patch. A string value sets the key, `null` deletes it, and an absent key is left alone. Bounds are enforced on the merged result.
    #[serde(rename = "metadata", skip_serializing_if = "Option::is_none")]
    pub metadata:
        Option<std::collections::HashMap<String, models::UpdateSessionRequestMetadataValue>>,
    /// Replaces the Session budget; null removes it. A finite ceiling may not be lower than estimated cost already consumed. Raising or removing the ceiling atomically requeues a turn paused on it.
    #[serde(
        rename = "budget",
        default,
    ...
    pub budget: Option<Option<Box<models::SessionBudget>>>,
}
