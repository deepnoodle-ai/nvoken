...
/// ImageInputBlock : An inline or URL image. A URL is fetched once during admission. nvoken stores the fetched bytes and never fetches it again. Admission sniffs the bytes, bounds them to 5242880 bytes and 8000 by 8000 pixels, and requires the selected model to declare the image modality and exact media type.
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct ImageInputBlock {
    #[serde(rename = "type")]
    pub r#type: Type,
    #[serde(rename = "source")]
    pub source: Box<models::ImageInputSource>,
}

impl ImageInputBlock {
    /// An inline or URL image. A URL is fetched once during admission. nvoken stores the fetched bytes and never fetches it again. Admission sniffs the bytes, bounds them to 5242880 bytes and 8000 by 8000 pixels, and requires the selected model to declare the image modality and exact media type.
    pub fn new(r#type: Type, source: models::ImageInputSource) -> ImageInputBlock {
        ImageInputBlock {
            r#type,
