...
pub async fn get_memory(
    configuration: &configuration::Configuration,
    memory_id: &str,
) -> Result<models::Memory, Error<GetMemoryError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_memory_id = memory_id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::Memory`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::Memory`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn list_memories(
    ...
    user_key: Option<&str>,
    query: Option<&str>,
    search_mode: Option<models::MemorySearchMode>,
    kind: Option<models::MemoryKind>,
    cursor: Option<&str>,
    limit: Option<u32>,
) -> Result<models::MemoryList, Error<ListMemoriesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    let uri_str = format!("{}/v1/memories", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);

    req_builder = req_builder.query(&[("agent_id", &p_query_agent_id.to_string())]);
