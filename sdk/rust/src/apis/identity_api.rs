...
pub async fn get_current_identity(
    configuration: &configuration::Configuration,
) -> Result<models::CurrentIdentityResponse, Error<GetCurrentIdentityError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::CurrentIdentityResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::CurrentIdentityResponse`")))),
        }
    } else {
        let content = resp.text().await?;
