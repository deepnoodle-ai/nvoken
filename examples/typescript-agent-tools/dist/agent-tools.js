...
import { Client, defineHostTool, defineJsonSchema, formatNvokenError, } from "@deepnoodle/nvoken";
...
try {
    ...
    const first = await chat.text("Look up order-42. Say its state and estimated delivery.");
    console.log(first);
    const second = await chat.text("What was the estimated delivery? Do not call the tool again.");
