...
const handle = await client.invoke({
  ...
  input: "hello",
  spec: { instructions: "help", model: { provider: "openai", id: "gpt-test" } },
});
...
""",
    ...
    snippet = re.search(
        r"<!-- public-quickstart:start -->\s*```ts\n(.*?)\n```\s*<!-- public-quickstart:end -->",
        installed_readme,
        re.DOTALL,
    )
    if snippet is None:
    ...
    if public_quickstart.stdout.strip() != "world":
    ...
    if packaged_command.stdout.strip() != "world":
        raise RuntimeError(f"packed command output = {packaged_command.stdout!r}")
    ...
    if packaged_from_file.stdout.strip() != "world":
    ...
    if (
        "NvokenError: invalid Runtime credential" not in credential_error
        or "category: 'authentication'" not in credential_error
        or "requestId: 'req_" not in credential_error
    ):
...
def check_examples(work: Path, artifact: Path, base_url: str) -> None:
    ...
    if quickstart.stdout.strip() != "world":
        raise RuntimeError(f"source quickstart output = {quickstart.stdout!r}")
