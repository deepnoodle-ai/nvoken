...
func newApp() *cli.App {
	app := cli.New("nvoken").
		...
		ExpandGroups(true)
	app.GlobalFlags(
		cli.String("base-url").Help("Runtime API base URL"),
		cli.String("config").Help("Path to JSON config file"),
		cli.Bool("json").Help("Emit stable JSON output"),
	)
...
func runtimeClient(command *cli.Context) (*nvoken.Client, error) {
	apiKey := strings.TrimSpace(os.Getenv("NVOKEN_API_KEY"))
	if apiKey == "" {
		return nil, fmt.Errorf("NVOKEN_API_KEY is required")
	}
	baseURL, err := resolveBaseURL(command.String("base-url"), command.String("config"))
	if err != nil {
		return nil, err
	}
	return nvoken.NewClient(baseURL, apiKey)
}
